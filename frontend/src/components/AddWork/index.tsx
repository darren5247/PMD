...
const AddWork: FC = (): JSX.Element => {
  ...
  const { control, formState, handleSubmit, watch, setValue } =
    useForm<TAddWorkForm>();
  const [userName, setUserName] = useState<string | null>(null);

  const [isTitle, setIsTitle] = useState(false);
  const [titleText, setTitleText] = useState<string | "">("");
  const [isTitleOverLimit, setIsTitleOverLimit] = useState(false);
  const [isAlternateTitleOverLimit, setIsAlternateTitleOverLimit] =
    useState(false);
  const [isVideoEmbedCodeOverLimit, setIsVideoEmbedCodeOverLimit] =
    useState(false);
  ...
  const [moodsOptionsLength, setMoodsOptionsLength] = useState<string | null>(
    null,
  );
  ...
  const [stylesOptionsLength, setStylesOptionsLength] = useState<string | null>(
    null,
  );
  ...
  const [themesOptionsLength, setThemesOptionsLength] = useState<string | null>(
    null,
  );
  ...
  const [elementsOptionsLength, setElementsOptionsLength] = useState<
    string | null
  >(null);
  ...
  const [studentAgeOptions, setStudentAgeOptions] = useState<string[] | null>(
    null,
  );

  const [studentTypes, setStudentTypes] = useState<string[]>([]);
  const [studentTypeOptions, setStudentTypeOptions] = useState<string[] | null>(
    null,
  );
  ...
  const [teachingTipsOptions, setTeachingTipsOptions] = useState<
    string[] | null
  >(null);
  const [teachingTipsOptionsLength, setTeachingTipsOptionsLength] = useState<
    string | null
  >(null);
  ...
  const [timeSignaturesOptions, setTimeSignaturesOptions] = useState<
    string[] | null
  >(null);
  const [timeSignaturesOptionsLength, setTimeSignaturesOptionsLength] =
  ...
  const [keySignaturesOptions, setKeySignaturesOptions] = useState<
    string[] | null
  >(null);
  const [keySignaturesOptionsLength, setKeySignaturesOptionsLength] = useState<
    string | null
  >(null);
  ...
  const [holidaysOptionsLength, setHolidaysOptionsLength] = useState<
    string | null
  >(null);
  ...
  useEffect(() => {
    ...
    if (addWorkData) {
      ...
      if (
        addWorkData[EWork.composers] &&
        addWorkData[EWork.composers].length > 0
      ) {
        setComposers(
          addWorkData[EWork.composers]?.map((item: IDataName) => item),
        );
      }
      if (
        addWorkData[EWork.publishers] &&
        addWorkData[EWork.publishers].length > 0
      ) {
        setPublishers(
          addWorkData[EWork.publishers]?.map((item: IDataName) => item),
        );
      }
      if (
        addWorkData[EWork.collections] &&
        addWorkData[EWork.collections].length > 0
      ) {
        setCollections(
          addWorkData[EWork.collections]?.map((item: IDataTitle) => item),
        );
      }
      ...
      if (
        addWorkData[EWork.elements] &&
        addWorkData[EWork.elements].length > 0
      ) {
        setElements(
          addWorkData[EWork.elements]?.map((item: IDataName) => item),
        );
      }
      ...
      if (
        addWorkData[EWork.studentAges] &&
        addWorkData[EWork.studentAges].length > 0
      ) {
        setStudentAges(
          addWorkData[EWork.studentAges]?.map((item: IDataTitle) => item),
        );
      }
      if (
        addWorkData[EWork.studentTypes] &&
        addWorkData[EWork.studentTypes].length > 0
      ) {
        setStudentTypes(
          addWorkData[EWork.studentTypes]?.map((item: IDataTitle) => item),
        );
      }
      if (
        addWorkData[EWork.teachingTips] &&
        addWorkData[EWork.teachingTips].length > 0
      ) {
        setTeachingTips(
          addWorkData[EWork.teachingTips]?.map((item: IDataTitle) => item),
        );
      }
      if (
        addWorkData[EWork.timeSignatures] &&
        addWorkData[EWork.timeSignatures].length > 0
      ) {
        setTimeSignatures(
          addWorkData[EWork.timeSignatures]?.map((item: IDataTitle) => item),
        );
      }
      if (
        addWorkData[EWork.keySignatures] &&
        addWorkData[EWork.keySignatures].length > 0
      ) {
        setKeySignatures(
          addWorkData[EWork.keySignatures]?.map((item: IDataTitle) => item),
        );
      }
      ...
      if (
        addWorkData[EWork.holidays] &&
        addWorkData[EWork.holidays].length > 0
      ) {
        setHolidays(
          addWorkData[EWork.holidays]?.map((item: IDataName) => item),
        );
      }
    ...
    const getComposers = async () => {
      const addWorkData = JSON.parse(
        localStorage.getItem("addWorkForm") || "{}",
      );
      try {
        ...
        if (fetchedData) {
          ...
          const filteredData = cleanComposers?.filter((composer: string) => {
            if (
              ...
              return !addWorkData[EWork.composers].some(
        ...
        if (fetchedData) {
          ...
          const filteredData = cleanPublishers?.filter((publisher: string) => {
            if (
              ...
              return !addWorkData[EWork.publishers].some(
          ...
          const filteredData = cleanCollections?.filter(
            (collection: string) => {
              if (
                ...
                return !addWorkData[EWork.collections].some(
    ...
    const getElements = async () => {
      ...
      try {
        ...
        if (fetchedData) {
          const cleanElements = fetchedData?.map(
            (item: IDataName) => item?.attributes?.name,
          );
          const filteredData = cleanElements?.filter((element: string) => {
            if (
              ...
              return !addWorkData[EWork.elements].some(
                (elementParsed: string) => elementParsed === element,
              );
            } else {
              return true;
            }
    ...
    const getMoods = async () => {
      ...
      try {
        ...
        if (fetchedData) {
          const cleanMoods = fetchedData?.map(
            (item: IDataTitle) => item?.attributes?.title,
          );
          const filteredData = cleanMoods?.filter((mood: string) => {
            if (
              addWorkData &&
              addWorkData[EWork.moods] &&
              addWorkData[EWork.moods].length > 0
            ) {
              return !addWorkData[EWork.moods].some(
                (moodParsed: string) => moodParsed === mood,
              );
            } else {
              return true;
            }
    ...
    const getStyles = async () => {
      ...
      try {
        ...
        if (fetchedData) {
          const cleanStyles = fetchedData?.map(
            (item: IDataTitle) => item?.attributes?.title,
          );
          const filteredData = cleanStyles?.filter((style: string) => {
            if (
              addWorkData &&
              addWorkData[EWork.styles] &&
              addWorkData[EWork.styles].length > 0
            ) {
              return !addWorkData[EWork.styles].some(
                (styleParsed: string) => styleParsed === style,
              );
            } else {
              return true;
            }
    ...
    const getThemes = async () => {
      ...
      try {
        ...
        if (fetchedData) {
          const cleanThemes = fetchedData?.map(
            (item: IDataTitle) => item?.attributes?.title,
          );
          const filteredData = cleanThemes?.filter((theme: string) => {
            if (
              addWorkData &&
              addWorkData[EWork.themes] &&
              addWorkData[EWork.themes].length > 0
            ) {
              return !addWorkData[EWork.themes].some(
                (themeParsed: string) => themeParsed === theme,
              );
            } else {
              return true;
            }
    ...
    const getTeachingTips = async () => {
      ...
      try {
        ...
        if (fetchedData) {
          const cleanTeachingTips = fetchedData?.map(
            (item: IDataTitle) => item?.attributes?.title,
          );
          const filteredData = cleanTeachingTips?.filter(
            (teachingTip: string) => {
              if (
                addWorkData &&
                addWorkData[EWork.teachingTips] &&
                ...
                return !addWorkData[EWork.teachingTips].some(
                  (teachingTipParsed: string) =>
                    teachingTipParsed === teachingTip,
                );
              } else {
                return true;
              }
    ...
    const getTimeSignatures = async () => {
      ...
      try {
        ...
        fetchedData.push(...data?.data);
        setTimeSignaturesOptionsLength(
          data?.meta?.pagination?.page * data?.meta?.pagination?.pageSize +
            "/" +
            data?.meta?.pagination?.total,
        );
        ...
        if (fetchedData) {
          const cleanTimeSignatures = fetchedData?.map(
            (item: IDataTitle) => item?.attributes?.title,
          );
          const filteredData = cleanTimeSignatures?.filter(
            (timeSignature: string) => {
              if (
                addWorkData &&
                addWorkData.timeSignatures &&
                addWorkData.timeSignatures.length > 0
              ) {
                return !addWorkData.TimeSignatures.some(
                  (timeSignatureParsed: string) =>
                    timeSignatureParsed === timeSignature,
                );
              } else {
                return true;
              }
    ...
    const getKeySignatures = async () => {
      ...
      try {
        ...
        if (fetchedData) {
          const cleanKeySignatures = fetchedData?.map(
            (item: IDataTitle) => item?.attributes?.title,
          );
          const filteredData = cleanKeySignatures?.filter(
            (KeySignature: string) => {
              if (
                addWorkData &&
                addWorkData.KeySignatures &&
                addWorkData.KeySignatures.length > 0
              ) {
                return !addWorkData.KeySignatures.some(
                  (KeySignatureParsed: string) =>
                    KeySignatureParsed === KeySignature,
                );
              } else {
                return true;
              }
    ...
    const getHolidays = async () => {
      ...
      try {
        ...
        if (fetchedData) {
          const cleanHolidays = fetchedData?.map(
            (item: IDataName) => item?.attributes?.name,
          );
          const filteredData = cleanHolidays?.filter((holiday: string) => {
            if (
              addWorkData &&
              addWorkData.holidays &&
              addWorkData.Holidays.length > 0
            ) {
              return !addWorkData.holidays.some(
                (holidayParsed: string) => holidayParsed === holiday,
              );
            } else {
              return true;
            }
  ...
  const handleMoods = (moodsRaw: string[]) => {
    ...
    const localData = localStorage.getItem("addWorkForm");
    const parsedLocalData = JSON.parse(localData as any);
    const dirtyData = dirtyMoods?.map(
      (item: IDataTitle) => item?.attributes?.title,
    );
    if (dirtyData && parsedLocalData) {
      const moodsParsed = parsedLocalData.moodsRaw;
      const filteredData = dirtyData.filter((item: string) => {
        return !moodsParsed?.some(
          (mood: IDataTitle) => mood?.attributes?.title === item,
        );
      });
      setMoodsOptions(filteredData);
    }
  ...
  const handleStyles = (stylesRaw: string[]) => {
    ...
    const localData = localStorage.getItem("addWorkForm");
    const parsedLocalData = JSON.parse(localData as any);
    const dirtyData = dirtyStyles?.map(
      (item: IDataTitle) => item?.attributes?.title,
    );
    if (dirtyData && parsedLocalData) {
      const stylesParsed = parsedLocalData.stylesRaw;
      const filteredData = dirtyData.filter((item: string) => {
        return !stylesParsed?.some(
          (style: IDataTitle) => style?.attributes?.title === item,
        );
      });
      setStylesOptions(filteredData);
    }
  ...
  const handleThemes = (themesRaw: string[]) => {
    ...
    const localData = localStorage.getItem("addWorkForm");
    const parsedLocalData = JSON.parse(localData as any);
    const dirtyData = dirtyThemes?.map(
      (item: IDataTitle) => item?.attributes?.title,
    );
    if (dirtyData && parsedLocalData) {
      const themesParsed = parsedLocalData.themesRaw;
      const filteredData = dirtyData.filter((item: string) => {
        return !themesParsed?.some(
          (theme: IDataTitle) => theme?.attributes?.title === item,
        );
      });
      setThemesOptions(filteredData);
    }
  ...
  const handleElements = (elementsRaw: string[]) => {
    ...
    const localData = localStorage.getItem("addWorkForm");
    const parsedLocalData = JSON.parse(localData as any);
    const dirtyData = dirtyElements?.map(
      (item: IDataName) => item?.attributes?.name,
    );
    if (dirtyData && parsedLocalData) {
      const elementsParsed = parsedLocalData.elementsRaw;
      const filteredData = dirtyData.filter((item: string) => {
        return !elementsParsed?.some(
          (element: IDataName) => element?.attributes?.name === item,
        );
      });
      setElementsOptions(filteredData);
    }
  ...
  const handleLevel = (levelRaw: string) => {
    ...
    const localData = localStorage.getItem("addWorkForm");
    const parsedLocalData = JSON.parse(localData as any);
    const dirtyData = CDirtyLevels?.map(
      (item: IDataTitle) => item?.attributes?.title,
    );
    if (dirtyData && parsedLocalData) {
      const levelParsed = parsedLocalData.levelRaw;
      const filteredData = dirtyData.filter((item: string) => {
        return !levelParsed?.some(
          (level: IDataTitle) => level?.attributes?.title === item,
        );
      });
  ...
  const handleStudentAges = (studentAgesRaw: string[]) => {
    ...
    const localData = localStorage.getItem("addWorkForm");
    const parsedLocalData = JSON.parse(localData as any);
    const dirtyData = CDirtyStudentsAges?.map(
      (item: IDataTitle) => item?.attributes?.title,
    );
    if (dirtyData && parsedLocalData) {
      const studentAgesParsed = parsedLocalData.studentAgesRaw;
      const filteredData = dirtyData.filter((item: string) => {
        return !studentAgesParsed?.some(
          (studentAges: IDataTitle) => studentAges?.attributes?.title === item,
        );
      });
  ...
  const handleStudentTypes = (studentTypesRaw: string[]) => {
    ...
    const localData = localStorage.getItem("addWorkForm");
    const parsedLocalData = JSON.parse(localData as any);
    const dirtyData = CDirtyStudentTypes?.map(
      (item: IDataTitle) => item?.attributes?.title,
    );
    if (dirtyData && parsedLocalData) {
      const studentTypesParsed = parsedLocalData.studentTypesRaw;
      const filteredData = dirtyData.filter((item: string) => {
        return !studentTypesParsed?.some(
          (studentTypes: IDataTitle) =>
            studentTypes?.attributes?.title === item,
        );
      });
  ...
  const handleTeachingTips = (teachingTipsRaw: string[]) => {
    ...
    const localData = localStorage.getItem("addWorkForm");
    const parsedLocalData = JSON.parse(localData as any);
    const dirtyData = dirtyTeachingTips?.map(
      (item: IDataTitle) => item?.attributes?.title,
    );
    if (dirtyData && parsedLocalData) {
      const teachingTipsParsed = parsedLocalData.teachingTipsRaw;
      const filteredData = dirtyData.filter((item: string) => {
        return !teachingTipsParsed?.some(
          (teachingTip: IDataTitle) => teachingTip?.attributes?.title === item,
        );
      });
      setTeachingTipsOptions(filteredData);
    }
  ...
  const handleTimeSignatures = (timeSignaturesRaw: string[]) => {
    ...
    const localData = localStorage.getItem("addWorkForm");
    const parsedLocalData = JSON.parse(localData as any);
    const dirtyData = dirtyTimeSignatures?.map(
      (item: IDataTitle) => item?.attributes?.title,
    );
    if (dirtyData && parsedLocalData) {
      const timeSignaturesParsed = parsedLocalData.timeSignaturesRaw;
      const filteredData = dirtyData.filter((item: string) => {
        return !timeSignaturesParsed?.some(
          (timeSignature: IDataTitle) =>
            timeSignature?.attributes?.title === item,
        );
      });
      setTimeSignaturesOptions(filteredData);
    }
  ...
  const handleKeySignatures = (keySignaturesRaw: string[]) => {
    ...
    const localData = localStorage.getItem("addWorkForm");
    const parsedLocalData = JSON.parse(localData as any);
    const dirtyData = dirtyKeySignatures?.map(
      (item: IDataTitle) => item?.attributes?.title,
    );
    if (dirtyData && parsedLocalData) {
      const keySignaturesParsed = parsedLocalData.keySignaturesRaw;
      const filteredData = dirtyData.filter((item: string) => {
        return !keySignaturesParsed?.some(
          (keySignature: IDataTitle) =>
            keySignature?.attributes?.title === item,
        );
      });
      setKeySignaturesOptions(filteredData);
    }
  ...
  const handleHolidays = (holidaysRaw: string[]) => {
    ...
    const localData = localStorage.getItem("addWorkForm");
    const parsedLocalData = JSON.parse(localData as any);
    const dirtyData = dirtyHolidays?.map(
      (item: IDataName) => item?.attributes?.name,
    );
    if (dirtyData && parsedLocalData) {
      const holidaysParsed = parsedLocalData.holidaysRaw;
      const filteredData = dirtyData.filter((item: string) => {
        return !holidaysParsed?.some(
          (holiday: IDataName) => holiday?.attributes?.name === item,
        );
      });
      setHolidaysOptions(filteredData);
    }
          ...
          payload: {
            message: error?.response?.data.error?.message
              ? error?.response?.data.error?.message == "Not Found"
                ? "A selection has been deleted. Please reset and try again."
  ...
  const prepareForm = (
    ...
    form[EWork.elements] = dirtyElements
      ...
      .map((item) => item.id);
    form[EWork.level] = CDirtyLevels?.filter((item: any) => {
      if (item.attributes.title === data.level) {
        return item;
      }
    }).map((item) => item.id);
    form[EWork.studentAges] = CDirtyStudentsAges?.filter((item: any) => {
    ...
    form[EWork.studentTypes] = CDirtyStudentTypes?.filter((item: any) => {
    ...
    form[EWork.keySignatures] = dirtyKeySignatures
      ...
      .map((item) => item.id);
    form[EWork.eras] = CDirtyEras?.filter((item: any) => {
    ...
    form[EWork.holidays] = dirtyHolidays
      ...
      .map((item) => item.id);
    form[EWork.instrumentations] = CDirtyInstrumentations?.filter(
      (item: any) => {
    ...
    const subscription = watch((data: any) => {
      ...
      if (localData) {
        ...
        if (data[EWork.hasTeacherDuet]) {
          data[EWork.hasTeacherDuet] = handleTransformString(
            data[EWork.hasTeacherDuet],
          );
        } else {
        ...
        localStorage.setItem(
          ...
          JSON.stringify({
            ...
            [EWork.alternateTitle]: data[EWork.alternateTitle]
              ? data[EWork.alternateTitle]
              : parsedLocalData.alternateTitle,
            [EWork.composers]: composers
              ? composers
            ...
            [EWork.studentAges]: studentAges
              ? studentAges
              : parsedLocalData.studentAges,
            [EWork.studentTypes]: studentTypes
              ? studentTypes
              : parsedLocalData.studentTypes,
            [EWork.teachingTips]: teachingTips
              ? teachingTips
              : parsedLocalData.teachingTips,
            [EWork.timeSignatures]: timeSignatures
              ? timeSignatures
              : parsedLocalData.timeSignatures,
            [EWork.keySignatures]: keySignatures
              ? keySignatures
              : parsedLocalData.keySignatures,
            [EWork.measureCount]: data[EWork.measureCount]
              ? data[EWork.measureCount]
              : parsedLocalData.measureCount,
            [EWork.holidays]: holidays ? holidays : parsedLocalData.holidays,
            [EWork.eras]: era ? era : parsedLocalData.era,
            [EWork.instrumentations]: instrumentation
              ? instrumentation
              : parsedLocalData.instrumentation,
            [EWork.yearPublished]: data[EWork.yearPublished]
              ? data[EWork.yearPublished]
              : parsedLocalData.yearPublished,
            [EWork.hasLyrics]: data[EWork.hasLyrics]
              ? data[EWork.hasLyrics]
              : parsedLocalData.hasLyrics,
            [EWork.hasTeacherDuet]: data[EWork.hasTeacherDuet]
              ? data[EWork.hasTeacherDuet]
              : parsedLocalData.hasTeacherDuet,
            [EWork.description]: data[EWork.description]
              ? data[EWork.description]
        ...
        localStorage.setItem(
          ...
          JSON.stringify({
            ...
            [EWork.studentAges]:
              studentAges && studentAges?.length > 0 ? studentAges : null,
            [EWork.studentTypes]:
              studentTypes && studentTypes?.length > 0 ? studentTypes : null,
            [EWork.teachingTips]:
              teachingTips && teachingTips?.length > 0 ? teachingTips : null,
            [EWork.timeSignatures]:
              timeSignatures && timeSignatures?.length > 0
                ? timeSignatures
                : null,
            [EWork.keySignatures]:
              keySignatures && keySignatures?.length > 0 ? keySignatures : null,
    ...
    <>
      ...
      >
        ...
        <ModalFeedback
          type={
            titleText
              ? "Add Work Disclaimer - " + titleText
              : firstControlValue[EWork.title]
                ? "Add Work Disclaimer - " + firstControlValue[EWork.title]
                : "Add Work Disclaimer - No Title Yet"
          }
      ...
      <div className="flex flex-col gap-y-6 mx-auto py-8">
        ...
        <div className="grow">
          <Field
            labelEl={<Label htmlFor={EWork.urlSpotify} label="Spotify Link" />}
        ...
        <div className="grow">
          <Field
            labelEl={
              <Label htmlFor={EWork.urlAppleMusic} label="Apple Music Link" />
            }
        ...
        <div className="flex flex-wrap gap-6 grow">
          ...
          {isLoadingStyles ? (
            <>
              <Divider className="mt-0 mb-0" />
              <p>
                Loading {stylesOptionsLength && stylesOptionsLength} Styles...
              </p>
            </>
          ) : (
            <>
          ...
          {isLoadingThemes ? (
            <>
              <Divider className="mt-0 mb-0" />
              <p>
                Loading {themesOptionsLength && themesOptionsLength} Themes...
              </p>
            </>
          ) : (
            <>
        ...
        <div className="flex flex-wrap gap-6 grow">
          {isLoadingElements ? (
            <p>
              Loading {elementsOptionsLength && elementsOptionsLength}{" "}
              Elements...
            </p>
          ) : (
            <>
        ...
        <div className="grow">
          {isLoadingTeachingTips ? (
            <>
              <p>
                Loading {teachingTipsOptionsLength && teachingTipsOptionsLength}{" "}
                Teaching Tips...
              </p>
            </>
          ) : (
        ...
        <div className="flex flex-wrap gap-6 grow">
          {isLoadingTimeSignatures ? (
            <>
              <p>
                Loading{" "}
                {timeSignaturesOptionsLength && timeSignaturesOptionsLength}{" "}
                Time Signatures...
              </p>
            </>
          ) : (
          ...
          {isLoadingKeySignatures ? (
            <>
              <p>
                Loading{" "}
                {keySignaturesOptionsLength && keySignaturesOptionsLength} Key
                Signatures...
              </p>
            </>
          ) : (
        ...
        <div className="grow">
          {isLoadingHolidays ? (
            <>
              <p>
                Loading {holidaysOptionsLength && holidaysOptionsLength}{" "}
                Holidays...
              </p>
            </>
          ) : (
        ...
        <div className="grow">
          <Field
            labelEl={<Label htmlFor={EWork.description} label="Description" />}
          ...
          <div className="flex flex-wrap items-center gap-2 mt-6">
            ...
            <div className="flex flex-col gap-y-4 bg-pmdGrayBright mt-3 px-6 py-4 rounded-lg w-full text-sm text-center">
              <p>
                <strong>NOTE:</strong>{" "}
                <em>
                  New works are not publicly visible until reviewed by staff!
                </em>
              </p>
              <p>
                You can view the status of your added works in{" "}
                <Link href={`/${EUrlsPages.WORKS_ADDED}`}>
                  <a className="text-pmdGray" title="Works Added">
                    Works Added
                  </a>
                </Link>
              </p>
            ...
            <p className="mt-3 mb-8 text-pmdGray text-sm">
              <em>
                By adding a work to Piano Music Database, you agree to the{" "}
            ...
            {(firstControlValue[EWork.title] || isTitle) &&
            composers &&
            composers.length > 0 &&
              ...
              composers && composers.length > 0 ? (
                firstControlValue[EWork.level] || level ? (
                  firstControlValue[EWork.eras] || era ? (
                    ...
                    <div className="flex flex-col gap-4 w-full">
                      ...
                      <p className="flex flex-col justify-center gap-2 bg-orange-500 px-3 py-10 rounded-lg w-full text-white text-lg text-center">
                        <span>
                          <em>
                            You must select an <strong>Era</strong>!
                          </em>
                        </span>
                        <a
                          title="Back to Top"
                  ...
                  <div className="flex flex-col gap-4 w-full">
                    ...
                    <p className="flex flex-col justify-center gap-2 bg-orange-500 px-3 py-10 rounded-lg w-full text-white text-lg text-center">
                      <span>
                        <em>
                          You must select a <strong>Level</strong>!
                        </em>
                      </span>
                      <a
                        title="Back to Top"
                ...
                <div className="flex flex-col gap-4 w-full">
                  ...
                  <p className="flex flex-col justify-center gap-2 bg-orange-500 px-3 py-10 rounded-lg w-full text-white text-lg text-center">
                    <span>
                      <em>
                        You must select at least one <strong>Composer</strong>!
                      </em>
                    </span>
                    <a
                      title="Back to Top"
