  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const filterOptions = CFilterOptionsItems.find(
          (opt) => opt.itemType === itemType,
        );
        ...
        if (apiTitleOrName === "title") {
          const itemsData = fetchedData.map(
        ...
        console.error(
          `Error fetching ${filterName.toLowerCase()} data:`,
          error,
        );
        setItems([]);
      } finally {
          ...
          <div className="flex flex-col justify-center items-center gap-2 mt-2 min-h-[80px] text-pmdGray text-center italic align-middle">
            ...
            {currentPage && totalPages ? (
              <div className="flex flex-col justify-center items-start gap-0 w-full h-full">
                <div
                  id="progressFilter"
                  className="bg-pmdRed rounded-full h-1"
                  style={{
                    width: `${(Number(currentPage) / Number(totalPages)) * 100}%`,
                    transition: "width 0.3s linear",
                    transformOrigin: "right",
                  }}
                ></div>
                <p>
                  {((Number(currentPage) / Number(totalPages)) * 100).toFixed(
                    2,
                  )}
                  %
                </p>
