...
const TableRowItem: FC<ITableRowItemProps> = ({ id, linkURL, linkAlt, col1, col2, col3, col4, col1Width, col2Width, col3Width, col4Width, pageBreakpoint }): JSX.Element => {
  const accountData: TUserAttributes = JSON.parse(localStorage.getItem('accountData') || '{}');
  const router = useRouter();
  const isBreakpoint = useMediaQuery(pageBreakpoint ? `(min-width: ${pageBreakpoint})` : '(min-width: 1250px)');
  ...
  return (
    <>
      {!isBreakpoint ? (
        <div key={`item${id.toString()}`} id={`item${id}`} className='flex flex-row items-stretch gap-0 even:bg-white odd:bg-pmdGrayBright border-pmdGrayLight border-b w-full grow'>
            ...
            <Link href={linkURL}>
              ...
              >
                <div className='flex flex-col flex-1 justify-center items-center w-full text-sm text-center grow'>
                  <div className='flex justify-center items-stretch w-full font-bold text-xl grow'>{col1}</div>
        ...
        <div key={`item${id.toString()}`} id={`item${id}`} className='group flex flex-row gap-0 even:bg-white odd:bg-pmdGrayBright border-pmdGrayLight border-b'>
          <div className='flex flex-col justify-start items-start gap-0 w-full text-sm text-left align-middle'>
            <Link href={linkURL}>
              ...
              >
                <div
                  className='flex flex-row flex-1 items-center gap-4 text-sm text-left align-middle'
                >
                  <div
                    className={cn(
                      'font-bold text-lg'
                    ...
                    style={
                      col1Width
                        ? { minWidth: col1Width, maxWidth: col1Width, width: col1Width }
                        : { minWidth: '230px', maxWidth: '230px', width: '230px' }
                    }
                  >
                    {col1}
                  </div>
                  {col2 ? (
                    <div
                      style={
                        col2Width
                          ? { minWidth: col2Width, maxWidth: col2Width, width: col2Width }
                          : { minWidth: '165px', maxWidth: '165px', width: '165px' }
                      }
                    >
                      {col2}
                    </div>
                  ...
                  {col3 ? (
                    <div
                      style={
                        col3Width
                          ? { minWidth: col3Width, maxWidth: col3Width, width: col3Width }
                          : { minWidth: '110px', maxWidth: '110px', width: '110px' }
                      }
                    >
                      {col3}
                    </div>
                  ...
                  {col4 ? (
                    <div
                      style={
                        col4Width
                          ? { minWidth: col4Width, maxWidth: col4Width, width: col4Width }
