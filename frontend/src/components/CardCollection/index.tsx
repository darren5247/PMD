...
export const CardCollection: FC<ICardCollectionProps> = ({
  ...
  return (
    <div id={`${id}-${title}`} className='flex flex-col justify-between items-center gap-y-2 bg-white shadow-musicCard pb-4 rounded-lg min-w-full sm:min-w-[580px] sm:min-w-auto max-w-[580px] text-center grow'>
      {!hideLabel && (
        <div className='bg-pmdGrayBright py-2 rounded-t-lg w-full text-pmdGray text-sm'><p>Collection</p></div>
      )}
      <div className='flex flex-col max-md:flex-col justify-center items-center px-4 text-center grow'>
        <div
          className={cn('mb-2 flex flex-row max-md:flex-col justify-center items-center text-center grow',
            {
              'mt-6': hideLabel
            },
            {
          ...
          <div className='flex flex-col justify-center items-center text-center grow'>
            {title ? (
              <div className='flex px-3 pb-2 grow'>
                <h3 className='flex justify-center items-center px-4 min-h-[96px] !text-2xl text-center grow'><strong>{title}</strong></h3>
              </div>
            ) : ''}
          </div>
        </div>
        {collection?.attributes.composers.data?.length ? (
          <p className='pr-1.5 pb-2'>Composed {collection?.attributes.composed_date ? (`in ${collection?.attributes.composed_date}`) : ''} <span>by </span>
              ...
              <span key={composer.attributes.name}>
                ...
                {collection?.attributes.composers.data?.length ? (
                  collection?.attributes.composers.data?.length > 3 ? (
                    i < collection?.attributes.composers.data?.length - 2 ? ', ' : i < collection?.attributes.composers.data?.length - 1 ? ', and ' : null
                  ) :
                    collection?.attributes.composers.data?.length < 2 ? '' : i < collection?.attributes.composers.data?.length - 1 ? ' and ' : null
                ) : ''}
              </span>
            ))}</p>
        ) : collection?.attributes.composed_date ? (<p className='pb-2'>Composed in {collection?.attributes.composed_date}</p>) : ''}
        {collection?.attributes.publishers.data?.length ? (
          <p className='pr-1.5 pb-2'>Published {collection?.attributes.published_date ? (`in ${collection?.attributes.published_date}`) : ''} <span>by </span>
              ...
              <span key={publisher.attributes.name}>
                ...
                {collection?.attributes.publishers.data?.length ? (
                  collection?.attributes.publishers.data?.length > 3 ? (
                    i < collection?.attributes.publishers.data?.length - 2 ? ', ' : i < collection?.attributes.publishers.data?.length - 1 ? ', and ' : null
                  ) :
                    collection?.attributes.publishers.data?.length < 2 ? '' : i < collection?.attributes.publishers.data?.length - 1 ? ' and ' : null
                ) : ''}
              </span>
            ))}</p>
        ) : collection?.attributes.published_date ? (<p className='pb-2'>Published in {collection?.attributes.published_date}</p>) : ''}
        {title ? (
          <p className='flex gap-2 mt-3 mb-4'>
            ...
            <Link href={`/${EUrlsPages.COLLECTION}/${handleTitleWithJustNumber(title)}?id=${id}`}><a className='flex gap-2' title={`View ${title}`} aria-label={`View ${title}`}><ImageNext src={IconOpen} width={18} height={18} /><strong>View</strong></a></Link>
          </p>
        ) : ''}
      </div>
    </div>
