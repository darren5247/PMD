...
const MultipleAutocomplete = forwardRef<HTMLDivElement, any>(
  ({ className, ...props }) => {
    const {
      name,
      setValues,
    ...
    const onClick = (e: any) => {
      ...
      if (values) {
        ...
        setValues([e.currentTarget.innerText]);
      };
    ...
    const onKeyDown = (e: KeyboardEvent) => {
      if (e.keyCode === 13) {
        ...
        if (newFilteredSuggestions.length > 0) {
          ...
          onFilter(newFilteredSuggestions.filter((el: string) => el !== newFilteredSuggestions[0]));
        };
              ...
              {filtered.map((suggestion, index) => {
                ...
                return (
                  <li
                    ...
                    key={suggestion}
                    onClick={onClick}
                  ><a className='text-pmdGrayDark hover:text-pmdGrayDark no-underline' title={suggestion}>{suggestion}</a></li>
                );
              })}
            </ul>
          );
        ...
        {values && (
          <div className='flex flex-wrap gap-2 mt-2 px-2'>
            {values?.map((el: string, i: number) => (
              <ChipRemovable onClose={() => handleDeleteChip(el)} title={el} key={i} />
            ))}
          </div>
        )}
      </div>
