...
const FieldSelectMultiple = forwardRef<HTMLDivElement, any>(({ className, hideViewAll = false, ...props }) => {
  ...
  useEffect(() => {
    ...
    if (values && values.length > 0 && suggestions && suggestions.length > 0) {
      for (let i = 0; i <= values.length; i++) {
        const index = suggestions.indexOf(values[i]);
        valuesCollection.push(index);
      };
      const max = valuesCollection.reduce((a, b) => Math.max(a, b), -Infinity);
      if (max >= 7) {
        setIsViewAll(() => true);
      };
    };
  ...
  return (
    <div>
      <div className={`flex flex-col gap-2 overflow-hidden ${isViewAll ? 'max-h-[100%]' : 'max-h-[175px]'}`}>
        {suggestions &&
      ...
      {values && !withoutChips && (
        <div className='flex flex-wrap gap-2 mt-[15px]'>
          {values.map((value: string) => (
            <ChipRemovable key={value} onClose={() => handleChange(value)} title={value} />
          ))}
        </div>
      )}
    </div>
