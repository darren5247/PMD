...
const FieldRadio = forwardRef<HTMLDivElement, any>(
  ({ viewAllThreshold, hideViewAll = false, ...props }) => {
    ...
    return (
      <div>
        <div
          className={`flex flex-col gap-2.5 overflow-hidden ${isViewAll ? "max-h-[100%]" : "max-h-[192px]"}`}
        >
          {suggestions &&
            suggestions
              .slice(
        ...
        {!isViewAll &&
          suggestions?.length > (viewAllThreshold ? viewAllThreshold : 8) && (
            <a
