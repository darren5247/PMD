...
const AddPublisher: FC = (): JSX.Element => {
  ...
  useEffect(() => {
    const addPublisherData = JSON.parse(
      localStorage.getItem("addPublisherForm") || "{}",
    );
    ...
    if (addPublisherData) {
      ...
      if (addPublisherData.urlSocialInstagram) {
        setValue(
          ...
          addPublisherData.urlSocialInstagram,
        );
      ...
      if (addPublisherData.urlSocialFacebook) {
        setValue(
          ...
          addPublisherData.urlSocialFacebook,
        );
      ...
      if (addPublisherData.urlSocialLinkedIn) {
        setValue(
          ...
          addPublisherData.urlSocialLinkedIn,
        );
      ...
      if (addPublisherData.urlSocialYouTube) {
        setValue(
          ...
          addPublisherData.urlSocialYouTube,
        );
      ...
      if (
        addPublisherData.collections &&
        addPublisherData.collections.length > 0
      ) {
        setCollections(
          addPublisherData.collections?.map((item: IDataName) => item),
        );
      }
    }
          ...
          const filteredData = cleanCollections?.filter(
            (collection: string) => {
              if (
                ...
                return !addPublisherData.collections.some(
  ...
  const handleData = async (form: any) => {
    try {
      ...
      router.push(`/${EUrlsPages.ADDED_PUBLISHER}`, undefined, {
        shallow: false,
      });
    } catch (error: any) {
      if (error?.response?.data) {
    ...
    <>
      ...
      >
        ...
        <ModalFeedback
          type={
            nameText
              ? "Add Publisher Disclaimer - " + nameText
              : firstControlValue[EPublisher.name]
                ? "Add Publisher Disclaimer - " +
                  firstControlValue[EPublisher.name]
                : "Add Publisher Disclaimer - No Name Yet"
          }
      ...
      <div className="flex flex-col gap-y-6 pt-2 pb-8">
        ...
        <div className="grow">
          <Field
            labelEl={
              <Label htmlFor={EPublisher.nationality} label="Nationality" />
            }
        ...
        <div className="grow">
          <Field
            labelEl={<Label htmlFor={EPublisher.excerpt} label="Description" />}
        ...
        <div className="grow">
          <Field
            labelEl={
              <Label htmlFor={EPublisher.urlSpotify} label="Spotify Link" />
            }
        ...
        <div className="grow">
          <Field
            labelEl={
              <Label htmlFor={EPublisher.urlWebsite} label="Custom Link" />
            }
        ...
        <div className="flex flex-wrap gap-6 grow">
          <div className="grow">
            <Field
              labelEl={
                <Label htmlFor={EPublisher.urlSocialX} label="X/Twitter Link" />
              }
          ...
          <div className="flex flex-wrap items-center gap-2 mt-6">
            ...
            <div className="flex flex-col gap-y-4 bg-pmdGrayBright mt-3 px-6 py-4 rounded-lg w-full text-sm text-center">
              <p>
                <strong>NOTE:</strong>{" "}
                <em>
                  New publishers are not publicly visible until reviewed by
                  staff!
                </em>
              </p>
              <p>
                You can view the status of your added publishers in{" "}
                <Link href={`/${EUrlsPages.PUBLISHERS_ADDED}`}>
                  <a className="text-pmdGray" title="Publishers Added">
                    Publishers Added
                  </a>
                </Link>
              </p>
            ...
            <p className="mt-3 mb-8 text-pmdGray text-sm">
              <em>
                By adding a publisher to Piano Music Database, you agree to the{" "}
