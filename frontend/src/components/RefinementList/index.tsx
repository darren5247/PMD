...
const transformItems: UseRefinementListProps['transformItems'] = (items) => {
  ...
  const hoist: hoistOptions = { 'Primary': 0, 'Early Elementary': 1, 'Late Elementary': 2, 'Early Intermediate': 3, 'Late Intermediate': 4, 'Advanced': 5, 'Master': 6 }
  if (items.length && (items[0].label == 'Primary' || items[0].label == 'Late Elementary' || items[0].label == 'Early Intermediate' || items[0].label == 'Late Intermediate' || items[0].label == 'Primary' || items[0].label == 'Advanced' || items[0].label == 'Early Elementary'))
...
const RefinementList: FC<IRefinementListProps> = ({
  ...
  return (
    ...
    >
      ...
      <p className='relative w-fit font-extrabold'>
        {title}
      </p>
      {isCollapsed && (
        <>
          <ul
            className={cn('list-none scrollbar mt-3 max-h-[266px] flex flex-col gap-y-2', {
