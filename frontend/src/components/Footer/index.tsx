...
const Footer: FC<IFooterProps> = ({ url, title }): JSX.Element => {
  ...
  const { control, handleSubmit, formState, reset } = useForm<TNewsletterForm>();
    ...
    [
      {
        title: 'Links',
        linkList: [
          { title: 'Start a Search', href: `/${EUrlsPages.SEARCH}`, target: '_self' },
          { title: 'Collections', href: `/${EUrlsPages.COLLECTIONS}`, target: '_self' },
          { title: 'Composers', href: `/${EUrlsPages.COMPOSERS}`, target: '_self' },
          { title: 'Publishers', href: `/${EUrlsPages.PUBLISHERS}`, target: '_self' },
          { title: 'Levels of Difficulty', href: `/${EUrlsPages.LEVELS}`, target: '_self' },
          { title: 'Elements of Piano', href: `/${EUrlsPages.ELEMENTS}`, target: '_self' },
      ...
      },
      {
        title: 'Help',
        linkList: [
          { title: 'Help Center', href: `/${EUrlsPages.HELP}`, target: '_self' },
          { title: 'Tutorials', href: `/${EUrlsPages.TUTORIALS}`, target: '_self' },
      ...
      },
      {
        title: 'Legal',
        linkList: [
          { title: 'Terms & Conditions', href: `/${EUrlsPages.TERMS_AND_CONDITIONS}`, target: '_self' },
          { title: 'Privacy Policy', href: `/${EUrlsPages.PRIVACY_POLICY}`, target: '_self' },
          { title: 'Cookie Policy', href: `/${EUrlsPages.COOKIE_POLICY}`, target: '_self' }
        ]
      },
      {
        title: 'Company',
        linkList: [
          ...
          { title: 'Contact', href: `/${EUrlsPages.CONTACT}`, target: '_self' }
        ]
      }
    ]
  ) : (
    [
      {
        ...
        linkList: [
          { title: 'FAQ', href: `/${EUrlsPages.FAQ}`, target: '_self' },
          { title: 'Email Newsletter', href: `/${EUrlsPages.NEWSLETTER}`, target: '_self' },
  ...
  const handleNewsletterSignUp: SubmitHandler<TNewsletterForm> = ({ name, nameLast, email }) => {
    if (!handleError({ name, email })) {
      handleSubscribe(name, nameLast, email);
    };
  };

  const handleSubscribe = async (name: string, nameLast: string, email: string) => {
    try {
      const brevoRequest = {
        'attributes': {
          'FIRSTNAME': name,
  ...
  return (
    <footer role='contentinfo' id='footer' className='flex flex-col justify-center items-center w-full no-print'>
      {!userId ? (
        <div className='flex flex-col justify-center items-center bg-[url("/lines.png")] bg-pmdRed bg-cover bg-no-repeat bg-bottom bg-local px-4 py-16 w-full text-white text-center'>
          <h2 id='try' className='flex justify-center items-center mb-2 font-medium text-center'><em>PMD Plus</em></h2>
          <p>All features <strong>FREE</strong> during Early Access</p>
          ...
          <Link href={`/${EUrlsPages.CREATE_ACCOUNT}`}><a title='Create Account' className='!px-10 !py-3 text-lg buttonwhite'>Create Account</a></Link>
        </div>
      ) : ''}
        ...
        <div className='flex md:flex-row flex-col justify-center md:justify-between items-center md:items-start gap-20 mt-20 md:mt-12 mb-24 md:mb-16 px-8 md:px-12 w-full max-w-[1250px] align-top'>
          <div className='flex flex-col justify-start items-start gap-2 text-white text-left align-middle'>
            <div className='flex flex-col items-start gap-2 max-w-[320px]'>
              <div className='flex gap-4'>
                <ImageNext src={IconBigEmail} height={48} width={48} className='min-w-[32px] min-h-[32px]' />
                <p className='text-white text-xl'><strong>Sign Up for Our <span className='flex'>Email Newsletter</span></strong></p>
              </div>
              <p className='text-pmdGrayLight text-sm text-left'><em>Sign up for our free email newsletter to receive updates from us about upcoming features, new music, deep dives into piano teaching, and more.</em></p>
            </div>
            <Form>
              <div className='flex flex-col gap-y-5 pt-3 max-w-[320px]'>
                ...
                <p className='text-[#727272] text-xs'>By signing up to our email newsletter, you agree to the <Link href={`/${EUrlsPages.TERMS_AND_CONDITIONS}`}><a className='text-[#727272] hover:text-pmdGray focus:text-pmdGrayBright active:!text-pmdGrayLight' title='Terms & Conditions'>Terms & Conditions</a></Link> and <Link href={`/${EUrlsPages.PRIVACY_POLICY}`}><a className='text-[#727272] hover:text-pmdGray focus:text-pmdGrayBright active:!text-pmdGrayLight' title='Privacy Policy'>Privacy Policy</a></Link>.</p>
                <div className='flex'>
                  <a
          ...
          <div className='flex flex-col justify-start items-center min-[890px]:items-start gap-10 text-white text-left align-middle'>
            <div className='flex max-[420px]:flex-col flex-wrap justify-between sm:justify-start items-start gap-10 max-[1086px]:max-w-[300px] text-white text-left align-middle'>
              {footerNavigation.map((navItem, idx) => (
                <div key={`${navItem.title}${idx}`}>
                  <p className='text-xl'><strong>{navItem.title}</strong></p>
                  <ul className='pt-[2px] text-sm leading-[30px] list-none'>
                    {navItem.linkList.map((link) => (
                      <li className='whitespace-nowrap' key={`${link.href + link.title}`}>
                        <Link href={link.href}><a title={link.title} target={link.target} className='text-pmdGrayLight focus:text-white hover:underline no-underline focus:no-underline'>{link.title}</a></Link>
                      </li>
                    ))}
                  </ul>
                </div>
            ...
            <div className='flex flex-col justify-start items-center md:items-start gap-2 w-min text-white text-left align-middle'>
              <p className='text-xl'><strong>Account</strong></p>
              <ul className='flex flex-row max-[889px]:flex-col bg-gradient-to-b from-white to-pmdGrayLight shadow-pmdRed shadow-tooltip border-pmdGray rounded-md divide-x divide-y divide-pmdGrayLight w-max text-sm leading-[30px] list-none'>
                {navLinks.map((item) => (
                  <li className='bg-transparent border-none rounded-md whitespace-nowrap' key={item.title}>
                    {item.onClick ? (
                      <a
                        title={item.title}
                        ...
                        tabIndex={0}
                      >
                        <div className='min-w-4 min-h-4'><ImageNext src={item.icon} alt='' height={16} width={16} /></div>
        ...
        <div className='bg-pmdRed w-full'>
          <div className='flex md:flex-row flex-col flex-wrap justify-center min-[840px]:justify-between items-center gap-16 mx-auto my-16 md:my-10 px-2 md:px-12 max-w-[1000px] md:max-w-[1250px] align-middle'>
            <div className='flex flex-wrap justify-center items-center gap-x-8 gap-y-6'>
              <Link href='https://facebook.com/pianomusicdatabase'><a title='Visit the Facebook for Piano Music Database' className='hover:opacity-75 focus:opacity-75 my-auto min-w-[18px] no-underline' target='_blank' rel='noopener noreferrer'>
                <ImageNext src={IconFacebook} alt='Facebook' />
              </a></Link>
              <Link href='https://instagram.com/pianomusicdatabase'><a title='Visit the Instagram for Piano Music Database' className='hover:opacity-75 focus:opacity-75 my-auto min-w-[18px] no-underline' target='_blank' rel='noopener noreferrer'>
                <ImageNext src={IconInstagram} alt='Instagram' />
              </a></Link>
              <Link href='https://threads.net/pianomusicdatabase'><a title='Visit the Threads for Piano Music Database' className='hover:opacity-75 focus:opacity-75 my-auto min-w-[18px] no-underline' target='_blank' rel='noopener noreferrer'>
                <ImageNext src={IconThreads} alt='Threads' />
              </a></Link>
              <Link href='https://pinterest.com/pianomusicdatabase'><a title='Visit the Pinterest for Piano Music Database' className='hover:opacity-75 focus:opacity-75 my-auto min-w-[18px] no-underline' target='_blank' rel='noopener noreferrer'>
                <ImageNext src={IconPinterest} alt='Pinterest' />
              </a></Link>
              <Link href='https://discord.gg/G8vEbn7ajF'><a title='Join the Discord for Piano Music Database' className='hover:opacity-75 focus:opacity-75 my-auto min-w-[18px] no-underline' target='_blank' rel='noopener noreferrer'>
            ...
            <div className='align-right flex flex-col gap-y-3 text-center md:text-right'>
              <Link href={EUrlsPages.HOMEPAGE}>
                <a title='Piano Music Database' className='flex md:flex-row flex-col justify-center items-center gap-4 hover:opacity-75 focus:opacity-75 md:pl-5 hover:underline no-underline focus:no-underline align-middle'>
                  <ImageNext
                    src={IconLogoWhite}
                  ...
                  />
                  <strong className='text-white text-2xl align-middle'>Piano Music Database</strong>
                </a>
              </Link>
              <div className='flex flex-row flex-wrap justify-center md:justify-end gap-x-2 gap-y-1 text-white text-xs text-center md:text-right'>
                <div><p>© 2021-{currentYear} Piano Music Database LLC.</p></div>
                {LIB_VERSION ? (<div><p><em className='pl-4 text-[10px]'>v{LIB_VERSION}</em></p></div>) : ''}
              </div>
