...
const TableRowWork: FC<ITableRowWorkProps> = ({
  id,
  title,
  composers,
  level,
  ...
  const [isRemovingListItemFrameOpen, setIsRemovingListItemFrameOpen] =
    useState<boolean>(false);
  const [isRemovingFavoriteItemFrameOpen, setIsRemovingFavoriteItemFrameOpen] =
    useState<boolean>(false);
  ...
  const workURL =
    ...
    EUrlsPages.WORK +
    "/" +
    encodeURIComponent(handleTitleWithJustNumber(titleText ? titleText : "")) +
    "-" +
    encodeURIComponent(
      handleTitleWithJustNumber(composerText ? composerText : ""),
    ) +
    "?id=" +
    id;
    ...
    const getFavorites = async () => {
      try {
        ...
        if (data !== null || data !== undefined || data?.data.length > 0) {
          const favoriteID =
            data?.data.length > 0 && data?.data[0].id ? data?.data[0].id : null;
          setFavId(favoriteID);
          const newIsFavorite =
    ...
    if (userId !== null && id !== null) {
      try {
        ...
        if (
          ...
          const favoriteID = dataAdded.data.data.id
            ? dataAdded.data.data.id
            : null;
          setFavId(favoriteID);
  ...
  const handleChangeNotes = async (newNotes: string) => {
    if (
      ...
      try {
        setIsLoadingFavorites(true);
        setIsSubmitAllowed(false);
        const { data } = await api.put(`list-works/${listWorkId}`, {
          data: {
            notes: newNotes,
          },
        });
  ...
  const handleRemoveListItemStart = async (listWorkId: number) => {
    if (listWorkId !== null && listWorkId !== undefined) {
      ...
      setTimeout(() => {
        const undoRemovalLink = document.getElementById(
          `undo-list-removal-${id}`,
        );
        if (undoRemovalLink) {
          ...
          <div className="flex flex-col justify-center items-center gap-4 p-4 w-full text-center align-middle">
            ...
            <div className="flex flex-wrap justify-center items-center gap-4">
              <a
                ...
                onClick={() => {
                  setTimeout(() => {
                    handleRemoveListItemFollowThrough(
                      listWorkId ? listWorkId : 0,
                    );
                  }, 0);
                }}
                onKeyDown={(e) => {
              ...
              >
                <div className="flex flex-col flex-1 justify-center items-center w-full text-sm text-center grow">
                  ...
                  <div className="flex items-center gap-3 mt-[12px]">
                    ...
                    {eras && typeof eras === "object" && eras.length ? (
                      <>
                        <span className="inline-block bg-pmdGray mx-0 w-px h-4" />
                      ...
                      <div className="flex flex-col gap-2 grow">
                        <div className="flex flex-row gap-2 w-full h-full">
                          ...
                          <button
                            ...
                            onClick={() => {
                              setNotesValue(
                                notesValueOld ? notesValueOld : null,
                              );
                              setIsEditing(false);
                            }}
                          >
                ...
                <div className="flex flex-col justify-center items-center gap-4 pt-3 border-pmdGrayLight border-t text-center align-middle">
                  ...
                  <div className="flex justify-center items-center w-full text-center align-middle">
                    <a
                      ...
                      onClick={() =>
                        listWorkId !== undefined &&
                        handleRemoveListItemStart(listWorkId)
                      }
                      onKeyDown={(e) => {
                        if (e.key === "Enter" || e.key === " ") {
                          listWorkId !== undefined &&
                ...
                <Link href={workURL}>
                  ...
                  >
                    <div className="flex flex-row flex-1 gap-4 text-sm text-left align-middle">
                      {accountData.id ? (
                        <div className="w-full min-w-[230px] max-w-[230px] font-bold text-lg">
                          {titleText}
                        </div>
                      ) : (
                        <div className="w-full min-w-[230px] font-bold text-lg">
                          {titleText}
                        </div>
                      )}
                      <div className="min-w-[165px] max-w-[165px]">
                        {composerText}
                      </div>
                      <span className="min-w-[110px] max-w-[110px]">
                        {level && level}
                      </span>
                      {eras ? (
