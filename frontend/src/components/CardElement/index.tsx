...
export const CardElement: FC<ICardElementProps> = ({
  ...
  return (
    <div
      id={`${id}-${name}`}
      className="flex flex-col justify-center items-center gap-y-2 bg-white shadow-elementCard pb-4 rounded-lg max-w-[252px] text-center"
    >
      {!hideLabel && (
      ...
      <div
        className={cn(
          "mb-2 flex flex-col justify-center items-center text-center ",
          {
            "mt-6": hideLabel,
          },
          {
      ...
      >
        ...
        {cat ? (
          <p className="flex justify-center items-center mt-1.5 mb-1 px-3 min-h-[42px] max-h-[42px] overflow-y-auto text-sm leading-[21px]">
            <strong>
              <em>{cat}</em>
            </strong>
          </p>
        ...
        {desc ? (
          <p className="flex mb-1 px-3 min-h-[135px] max-h-[135px] overflow-y-auto text-sm leading-[21px]">
            {desc}
          </p>
        ) : (
          ""
        ...
        {name ? (
          <div className="flex gap-2 mt-3 mb-2">
            ...
            <Link
              href={`/${EUrlsPages.ELEMENT}/${encodeURIComponent(name)}?id=${id}`}
            >
              <a
