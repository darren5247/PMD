    ...
    >
      <div className="flex flex-row justify-center items-center gap-0 w-full">
        <div className="flex flex-col justify-center items-stretch gap-2 pb-2 border-pmdGray border-r w-full">
          <p className="bg-pmdGray p-1 border-pmdGray border-b rounded-tl text-white capitalize align-middle">
            <strong>{label}</strong>
          </p>
        ...
        <a
          ...
          title={
            label == "Search Term"
              ? "Remove Search: " + decodeURIComponent(title)
              : "Remove Filter: " + decodeURIComponent(title)
          }
          onClick={onReset}
          onKeyDown={(e) => {
