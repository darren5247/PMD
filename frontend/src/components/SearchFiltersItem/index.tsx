...
const SearchFiltersItem: FC<ISearchFiltersItemProps> = ({
  itemType,
  itemTypeTitleOrName,
  className,
  mobileBreakpoint,
  ...
  const { query } = router;
  const [facetedFilters, setFacetedFilters] = useState<IFilterOption[] | null>(
    null,
  );
  ...
  const [elementCategoryFilter, setElementCategoryFilter] = useState<
    string | string[] | null
  >(null);
  const [levelFilter, setLevelFilter] = useState<string | string[] | null>(
    null,
  );
  ...
  useEffect(() => {
    if (facetedFilters === null) {
      if (
        ...
        // fetchData();
        setFacetedFilters(
          CFilterOptionsItems.filter((item) => {
  ...
  useEffect(() => {
    ...
    setFiltering(!!(category || level || era));
    ...
    CFilterKeys.filter((filterKey) => filterKey.itemType === itemType).forEach(
      (filterKey) => {
  ...
  const resetSpecificFilter = (
    ...
    const resetFilter = (
      ...
      setFilter: (value: string | string[] | null) => void,
    ) => {
  ...
  return (
    ...
    >
      <SearchFormItem
        pageLink={itemTypeState?.pageLink ? itemTypeState?.pageLink : ""}
      />
      {(
          ...
          <div className="flex flex-col justify-center items-center gap-4 mx-auto w-full align-middle">
            <div className="flex flex-col justify-center items-center gap-2 mx-auto w-full align-middle">
              <h2 className="font-bold text-lg">
                Current Filter{isFilteringMultiple ? "s" : ""}
              </h2>
                ...
                {!(textQuery && textQuery.length) ||
                  (isFilteringMultiple && (
                    <div className="flex justify-center items-center mx-auto w-full whitespace-nowrap">
                      <h2
                        id="and-searchterm"
                        className="relative pr-[.3em] pl-[.3em] font-extrabold text-pmdGray text-base min-[380px]:text-lg md:text-xl italic leading-[22px] md:leading-[29px] tracking-[0.1px] min-[380px]:tracking-[0.546667px]"
                      >
                        and
                      </h2>
                    </div>
                ...
                {isFiltering ? (
                  ...
                  >
                    {(() => {
                      ...
                      const filterConfig = CFilterKeys.find(
                        (fk) => fk.itemType === itemType,
                      );
                      if (
                        !filterConfig ||
                        !Array.isArray(filterConfig.filterOptions)
                      )
                        return null;
                      ...
                      return filterConfig.filterOptions
                        ...
                        .map((filterOption, index, array) => (
                          <div
                            key={`${filterOption.filter}-${index}`}
                            className="flex flex-col justify-center items-center gap-2 mx-auto w-full text-center"
                          >
                            <div className="flex flex-wrap justify-center items-stretch gap-2 mx-auto w-full max-w-[279px] text-center">
                              <ChipSearchItem
            ...
            <div className="flex justify-center items-center align-middle">
              <a
                ...
                tabIndex={0}
              >
                {isFilteringMultiple ? "Remove all filters." : "Remove filter."}
              </a>
            </div>
          </div>
        </div>
