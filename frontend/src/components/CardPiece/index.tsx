...
export const CardPiece: FC<ICardPieceProps> = ({
  piece,
  hideLabel,
}): JSX.Element => {
  ...
  if (piece.attributes.title) {
    ...
    workURL =
      "/" + EUrlsPages.WORK + "/" + workTitleEncoded + "?id=" + piece.id;
  } else {
    titleSaved = "";
    workTitleDisplay = "";
    workTitleEncoded = "";
      ...
      <div className="flex flex-row max-md:flex-col justify-center items-center text-center">
        <div className="flex flex-col justify-center items-center text-center">
          <div className="gap-y-1 px-4">
            <h3 className="flex justify-center items-center min-h-[96px] !text-2xl text-center">
              <strong>{titleSaved}</strong>
            </h3>
            <p>{workComposers}</p>
          </div>
          {piece.attributes.level?.data?.attributes.title && (
            <p className="mb-4 px-4">
              <strong>
                <em>{piece.attributes.level?.data?.attributes.title}</em>
              </strong>
            </p>
          )}
        </div>
      ...
      <div className="mt-3 mb-2">
        <Link href={workURL}>
          <a
            ...
            title={`View ${workTitleDisplay}`}
            aria-label={`View ${workTitleDisplay}`}
          >
            <ImageNext src={IconOpen} width={20} height={20} />
            <strong>View</strong>
          </a>
        </Link>
      </div>
