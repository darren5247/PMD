...
const ModalEdit: FC<IModalEditProps> = ({
  ...
  useEffect(() => {
    const editData = JSON.parse(
      localStorage.getItem("EditForm" + typeDisplay + fieldDisplay) || "{}",
    );
    if (editData) {
  ...
  const handleError = (form: any) => {
    ...
    if (!form.newContent) {
      newErrors.push({
        name: "newContent",
        message: "Enter a new " + fieldDisplay,
      });
    }
    ...
    >
      <div className="flex gap-2 py-4 pr-12 pl-5">
        ...
        <p className="pt-1 text-pmdGrayDark">
          <strong>{fieldDisplay && "Edit " + fieldDisplay}</strong>
        </p>
      </div>
      <div className="px-4 pt-4 pb-6 border-pmdGrayLight border-t max-h-screen overflow-auto text-black scrollbar">
        <Form className="flex flex-col">
          <div className="flex flex-col gap-6 pt-2 pb-8">
            <div className="grow">
              <Label label={"Current " + fieldDisplay} />
              <p>
                {currentContent ? currentContent : "Unknown " + fieldDisplay}
              </p>
            </div>
            <div className="grow">
              <Field
            ...
            <div className="grow">
              <Field
                labelEl={<Label htmlFor="reason" label="Reason (Optional)" />}
                name="reason"
                component={TextArea}
                rows={7}
                control={control}
          ...
          <div className="flex flex-col gap-y-4 bg-pmdGrayBright mt-1 mb-6 px-6 py-4 rounded-lg w-full text-sm text-center">
            <p>
              <strong>NOTE:</strong>{" "}
              <em>Edits are not publicly visible until reviewed by staff!</em>
            </p>
            <p>
              You can view your edits in{" "}
              <Link href={`/${EUrlsPages.EDIT_HISTORY}`}>
                <a className="text-pmdGray" title="Edit History">
                  Edit History
                </a>
              </Link>
            </p>
          ...
          <p className="mt-3 mb-8 text-pmdGray text-sm">
            <em>
              By sending edits to Piano Music Database, you agree to the{" "}
        ...
        {edits &&
          edits.length > 0 &&
          edits.filter((edit) => edit.attributes.status !== "Rejected").length >
            0 &&
          edits[0].id && (
            <>
