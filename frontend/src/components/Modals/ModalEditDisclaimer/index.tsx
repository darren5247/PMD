...
const ModalEditDisclaimer: FC<IModalEditDisclaimerProps> = ({
    ...
    const { control, handleSubmit, formState, setValue, watch } = useForm<TEditDisclaimerForm>();
    ...
    useEffect(() => {
        const addFeedbackData = JSON.parse(localStorage.getItem('addFeedbackForm') || '{}');
        if (addFeedbackData) {
    ...
    const handleAddFeedback: SubmitHandler<TEditDisclaimerForm> = ({ feedbackText, feedbackEmail }) => {
        if (!handleError({
            feedbackText,
            feedbackEmail
        })) {
    ...
    return (
        ...
        >
            <div className='flex gap-2 py-4 pr-12 pl-5'>
                <ImageNext src={IconFeedback} height={22} width={22} className='min-w-[22px] min-h-[22px]' />
                <p className='pt-1 text-pmdGrayDark'><strong>Send Edits</strong></p>
            </div>
            <div className='px-4 pt-4 pb-6 border-pmdGrayLight border-t max-h-screen overflow-auto text-black scrollbar'>
                <div className='flex flex-col gap-3 mt-3 mb-6 text-pmdGrayDark'>
                    <p>Use this form to tell us about edits you would like to make to anything previously submitted.</p>
                    <p>Be sure to include any relevant information that will help us identify where your edits belong.</p>
                    <p>We will follow up with you when we have applied your edits.</p>
                    <p className='text-sm'><span className='font-bold text-pmdRed text-base'>*</span> indicates a required field</p>
                </div>
                ...
                <Form className='flex flex-col'>
                    <div className='flex flex-col gap-6 pt-2 pb-8'>
                        ...
                        {!userName ? (
                            <div className='grow'>
                                <Field
                                    labelEl={<Label
                                        htmlFor={EFeedback.feedbackEmail}
                                        label='Email'
                                    />}
                    ...
                    <p className='mt-3 mb-8 text-pmdGray text-sm'><em>By sending this form to Piano Music Database, you agree to the <Link href={`/${EUrlsPages.TERMS_AND_CONDITIONS}`}><a className='text-pmdGray' title='Terms and Conditions'>terms and conditions</a></Link>.</em></p>
