...
export const VideoPlayer: FC<IVideoPlayerProps> = ({
  ...
  const { cookieConsent, shouldShowBanner, setShouldShowBanner } = useCookieConsent();
  ...
  return (
    ...
    >
      <iframe
        src={`https://youtube.com/embed/` + (YTVideoIdArray ? `${YTVideoIdArray[0]}` : '') + (startSecond ? `?start=${startSecond}` : '')}
        style={{ border: 0 }}
