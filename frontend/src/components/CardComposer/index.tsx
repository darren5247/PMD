...
export const CardComposer: FC<ICardComposerProps> = ({
  ...
  return (
    <div id={`${id}-${name}`} className='flex flex-col justify-center items-center gap-y-2 bg-white shadow-musicCard pb-4 rounded-lg min-w-full sm:min-w-[580px] sm:min-w-auto max-w-[580px] text-center'>
      {!hideLabel && (
        <div className='bg-pmdGrayBright py-2 rounded-t-lg w-full text-pmdGray text-sm'><p>Composer</p></div>
      )}
      <div
      ...
      {birth_year && death_year ? (<p>Born {birth_year}, Died {death_year}</p>) : ''}
      ...
      {name ? (
        <p className='flex gap-2 mt-3 mb-4'>
          ...
          <Link href={`/${EUrlsPages.COMPOSER}/${checkNumberName(name)}?id=${id}`}><a className='flex gap-2' title={`View ${name}`} aria-label={`View ${name}`}><ImageNext src={IconOpen} width={18} height={18} /><strong>View</strong></a></Link>
        </p>
      ) : ''}
    </div>
  );
