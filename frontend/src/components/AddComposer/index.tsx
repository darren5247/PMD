...
const AddComposer: FC = (): JSX.Element => {
  ...
  const [firstControlValue, setFirstControlValue] = useState(defaultComposerValues);
  ...
  const { control, handleSubmit, formState, setValue, watch } = useForm<TAddComposerForm>();
  ...
  useEffect(() => {
    const addComposerData = JSON.parse(localStorage.getItem('addComposerForm') || '{}');
    ...
    if (addComposerData) {
      ...
      if (addComposerData.urlSocialInstagram) {
        setValue(EComposer.urlSocialInstagram, addComposerData.urlSocialInstagram);
      ...
      if (addComposerData.urlSocialFacebook) {
        setValue(EComposer.urlSocialFacebook, addComposerData.urlSocialFacebook);
      ...
      if (addComposerData.urlSocialLinkedIn) {
        setValue(EComposer.urlSocialLinkedIn, addComposerData.urlSocialLinkedIn);
      ...
      if (addComposerData.collections && addComposerData.collections.length > 0) {
        setCollections(addComposerData.collections?.map((item: IDataName) => item));
      };
    };
      ...
      try {
        ...
        if (fetchedData) {
          ...
          const filteredData = cleanCollections?.filter((collection: string) => {
            if (addComposerData && addComposerData.collections && addComposerData.collections.length > 0) {
              return !addComposerData.collections.some((collectionParsed: string) => collectionParsed === collection)
            } else {
              return true;
            };
          });
  ...
  const handleData = async (form: any) => {
    try {
      ...
      router.push(`/${EUrlsPages.ADDED_COMPOSER}`, undefined, { shallow: false });
    } catch (error: any) {
      if (error?.response?.data) {
        dispatch({
          type: ENotificationActionTypes.SET_MESSAGE,
      ...
      if (localData) {
        ...
        localStorage.setItem(
          ...
          JSON.stringify({
            ...parsedLocalData,
            ...data,
            [EComposer.name]: data[EComposer.name] ? data[EComposer.name] : parsedLocalData.name,
            [EComposer.birth_year]: data[EComposer.birth_year] ? data[EComposer.birth_year] : parsedLocalData.birth_year,
            [EComposer.death_year]: data[EComposer.death_year] ? data[EComposer.death_year] : parsedLocalData.death_year,
            [EComposer.nationality]: data[EComposer.nationality] ? data[EComposer.nationality] : parsedLocalData.nationality,
            [EComposer.ethnicity]: data[EComposer.ethnicity] ? data[EComposer.ethnicity] : parsedLocalData.ethnicity,
            [EComposer.gender]: data[EComposer.gender] ? data[EComposer.gender] : parsedLocalData.gender,
            [EComposer.pronouns]: data[EComposer.pronouns] ? data[EComposer.pronouns] : parsedLocalData.pronouns,
            [EComposer.excerpt]: data[EComposer.excerpt] ? data[EComposer.excerpt] : parsedLocalData.excerpt,
            [EComposer.urlSpotify]: data[EComposer.urlSpotify] ? data[EComposer.urlSpotify] : parsedLocalData.urlSpotify,
            [EComposer.urlAppleMusic]: data[EComposer.urlAppleMusic] ? data[EComposer.urlAppleMusic] : parsedLocalData.urlAppleMusic,
            [EComposer.urlWebsite]: data[EComposer.urlWebsite] ? data[EComposer.urlWebsite] : parsedLocalData.urlWebsite,
            [EComposer.urlSocialInstagram]: data[EComposer.urlSocialInstagram] ? data[EComposer.urlSocialInstagram] : parsedLocalData.urlSocialInstagram,
            [EComposer.urlSocialFacebook]: data[EComposer.urlSocialFacebook] ? data[EComposer.urlSocialFacebook] : parsedLocalData.urlSocialFacebook,
            [EComposer.urlSocialX]: data[EComposer.urlSocialX] ? data[EComposer.urlSocialX] : parsedLocalData.urlSocialX,
            [EComposer.urlSocialLinkedIn]: data[EComposer.urlSocialLinkedIn] ? data[EComposer.urlSocialLinkedIn] : parsedLocalData.urlSocialLinkedIn,
            [EComposer.urlSocialYouTube]: data[EComposer.urlSocialYouTube] ? data[EComposer.urlSocialYouTube] : parsedLocalData.urlSocialYouTube,
            [EComposer.collections]: collections ? collections : parsedLocalData.collections
          })
        );
      } else {
        localStorage.setItem(
          ...
          JSON.stringify({
            ...data,
            [EComposer.collections]: (collections && collections?.length > 0) ? collections : null
          })
        );
      };
  ...
  return (
    <>
      <div id='feedback' className='flex flex-row flex-wrap justify-center items-center gap-x-5 gap-y-2 bg-pmdGrayBright mb-3 px-3 py-8 rounded-lg w-full text-center'>
        ...
        <ModalFeedback
          type={nameText ? ('Add Composer Disclaimer - ' + nameText) : firstControlValue[EComposer.name] ? ('Add Composer Disclaimer - ' + firstControlValue[EComposer.name]) : 'Add Composer Disclaimer - No Name Yet'}
        ...
        <>
          <div className='flex flex-wrap items-center gap-2 mt-6'>
            ...
            <div className='flex flex-col gap-y-4 bg-pmdGrayBright mt-3 px-6 py-4 rounded-lg w-full text-sm text-center'>
              <p><strong>NOTE:</strong> <em>New composers are not publicly visible until reviewed by staff!</em></p>
              <p>You can view the status of your added composers in <Link href={`/${EUrlsPages.COMPOSERS_ADDED}`}><a className='text-pmdGray' title='Composers Added'>Composers Added</a></Link></p>
            </div>
            <p className='mt-3 mb-8 text-pmdGray text-sm'><em>By adding a composer to Piano Music Database, you agree to the <Link href={`/${EUrlsPages.TERMS_AND_CONDITIONS}`}><a className='text-pmdGray' title='Terms and Conditions'>terms and conditions</a></Link>.</em></p>
