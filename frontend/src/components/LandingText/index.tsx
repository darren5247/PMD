...
const LandingText: FC<ILandingTextProps> = ({ queries }): JSX.Element => {
  ...
  if (queries) {
    const matchLanding: RegExpMatchArray | null = queries.match(/landing=(.*?)(?:&|$)/);
    if (matchLanding) {
    ...
    const matchLandingLine2: RegExpMatchArray | null = queries.match(/landing2=(.*?)(?:&|$)/);
    ...
    <>
      {landingTextLine1 && (
        <div className='bg-pmdGrayLight shadow-workNav px-4 py-8 w-full text-xl text-center'>
          <p><strong>{`${landingTextLine1}`}</strong></p>
          {landingTextLine2 && (
            <p><strong>{`${landingTextLine2}`}</strong></p>
          )}
        </div>
      )}
    </>
...
export const getServerSideProps: GetServerSideProps<ILandingTextProps> = async ({ req }) => {
