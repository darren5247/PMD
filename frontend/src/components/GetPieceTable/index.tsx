...
export const GetPieceTable: FC<IGetPieceTableProps> = ({ workIds }): JSX.Element => {
      ...
      const getAPIPieces = async () => {
        try {
          ...
          const { data } = await api.get(
            `works?pagination[page]=1&pagination[pageSize]=${workIds ? workIds.split(',').length : '1'}&populate[composers][fields][0]=name&populate[level][fields][1]=title&populate[eras][fields][2]=name&fields[0]=title${workIds ? workIds.split(',').map((id: string) => (`&filters[$and][0][id][$eq]=${id.trim()}`)).join('') : '0'}&publicationState=preview`
          );
          ...
          if (
            ...
            for (let i = page + 1; i <= pageCount; i++) {
              let response = await api.get(
                `works?pagination[page]=${i}&pagination[pageSize]=${workIds ? workIds.split(',').length : '1'}&populate[composers][fields][0]=name&populate[level][fields][1]=title&populate[eras][fields][2]=name&fields[0]=title${workIds ? workIds.split(',').map((id: string) => (`&filters[$and][0][id][$eq]=${id.trim()}`)).join('') : '0'}&publicationState=preview`
              );
      ...
      {(!isLoadingPieces && pieces && pieces.length > 0) && (
        <div className='flex flex-row flex-wrap justify-center items-center gap-3 my-4 w-full max-w-[1200px] md:max-w-[816px] text-center align-middle'>
          <div className='mt-5 mb-4 px-1'>
            <Works works={Object.values(pieces).slice(0, 10).map(piece => ({
              id: piece.id,
              ...
              eras: piece.attributes.eras?.data?.map(composer => composer.attributes.name) || null
            }))} />
          </div>
        </div>
      )}
