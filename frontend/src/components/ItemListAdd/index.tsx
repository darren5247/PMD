...
const ItemListAdd: FC<IItemListAdd> = ({
  index,
  workId,
  list,
}): JSX.Element => {
  ...
  const getHighestOrder = async (listId: number) => {
    if (listId !== null && listId !== undefined && listId !== 0) {
      try {
        ...
        if (
          data?.data &&
          data?.data.attributes["list-works"]?.data.length > 0
        ) {
          const orders = data.data.attributes["list-works"].data.map(
            (work: any) => work.attributes.order,
          );
  ...
  return (
    <div
      key={index}
      className="flex flex-row max-[410px]:flex-col justify-center items-stretch gap-0 mx-2 text-center align-middle"
    >
      <a
        title={list.attributes.title}
        className="flex flex-col items-center bg-white hover:bg-pmdGrayBright shadow-header p-4 border border-pmdGrayBright rounded-l-md h-fill no-underline transition-all cursor-pointer grow"
        tabIndex={0}
        onClick={() => {
        ...
        <div className="flex max-[410px]:flex-col justify-between items-center gap-4 my-auto">
          <div className="min-[410px]:min-w-44 max-w-44 text-left">
            <p
              className={cn(
                ...
                {
                  "break-all": list.attributes.title
                    .split(" ")
                    .some((word) => word.length > 8),
                },
              )}
            >
              {list.attributes.title.length > 20
          ...
          <div className="flex flex-row justify-start items-start gap-2 text-pmdGray text-sm text-left">
            {list.attributes.visibility.data ? (
              ...
              list.attributes.visibility.data.id === 2 ? (
                list.attributes.visibility.data.id === 1 ? (
        ...
        href={
          ...
          encodeURIComponent(
            handleTitleWithJustNumber(
              list.attributes.title ? list.attributes.title : "",
            ),
          ) +
          "?id=" +
          list.attributes.uid
      ...
      >
        <a
          title={list.attributes.title}
          className="flex flex-row justify-center items-center gap-2 bg-white hover:bg-pmdGrayBright shadow-header px-4 py-4 max-[410px]:py-3 border border-pmdGrayBright rounded-r-md h-fill text-sm text-center no-underline transition-all cursor-pointer grow"
        >
          <div className="min-w-4 min-h-4">
            <ImageNext src={IconOpen} alt="" height={16} width={16} />
          </div>{" "}
          View
        </a>
      </Link>
