...
const checkJwtOrRedirect = () => {
  ...
  if (!jwt) {
    handleRedirect("Session invalid. Please log in again.");
  }
  ...
  try {
    if (!jwt) {
      ...
      if (payload.exp && Date.now() >= payload.exp * 1000) {
        handleRedirect("Session expired. Please log in again.");
      }
    }
  } catch {
    handleRedirect("Session errored. Please log in again.");
  }
};
