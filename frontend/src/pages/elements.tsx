...
const ElementsPage: NextPage<IElementsPageProps> = ({ prevUrl, queries }) => {
  ...
  const handleIsOpenElementCategoriesModal = () => setIsOpenElementCategoriesModal(!isOpenElementCategoriesModal);
  const [isOpenElementCategoriesModal, setIsOpenElementCategoriesModal] = useState(false);
  ...
  return (
    ...
    >
      <div className='flex flex-col justify-center items-center text-center'>
        ...
        <p className='mt-3 max-w-[816px] text-justify'>Elements are the fundamental pedagogical concepts and techniques contained in each piece of piano music in the database. Knowing the elements of a particular piece helps you understand how difficult the piece is to play and what a student needs to know in order to successfully play it.</p>

        <hr id='category-selector' className='my-8 border-pmdGrayLight border-t-2 w-full' />

        <p className='mt-6 mb-3 text-sm text-center italic'>Click the button below to see a list of element categories. <br />Then, select a category to explore all of those elements.</p>
        <button
        ...
        />
        <div id='elements' className='flex flex-col flex-wrap justify-center items-center mt-8'>
          {isLoadingElements ? (
            (currentCategory) ? (
            ...
            (elements && elements.length && currentCategory ? (
              <div className='flex flex-col justify-center items-center gap-2 align-middle'>
                <em className='text-xl'>Showing elements for category: <strong>{currentCategory}</strong></em>
...
export const getServerSideProps: GetServerSideProps<IElementsPageProps> = async ({ req }) => {
  return {
    props: {
