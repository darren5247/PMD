...
const CollectionsAddedPage: NextPage<ICollectionsAddedPageProps> = ({ prevUrl }) => {
    const router = useRouter();
    const { query } = router;
    const [userName, setUserName] = useState<string | null>(null);
    const [userEmail, setUserEmail] = useState<string | null>(null);
    let [collections, setCollections] = useState<IStrapiCollection[]>([]);
    ...
    useEffect(() => {
        ...
        getCollections();
    }, [dispatch, router, currentPage, pageSize, query, setCollections, setIsLoadingCollections, userEmail]
    );
                        ...
                        (collections && collections.length) ? (
                            <div>
                                ...
                                {collections.map((collection) => (
                                    <div className='flex flex-row gap-4 even:bg-white odd:bg-pmdGrayBright mt-6 p-4 text-left' key={collection.id}>
                                        <div className='flex flex-col gap-4 text-left'>
                                            <p><strong>#{collection.id}</strong></p>
                                            <Link href={`/${EUrlsPages.COLLECTION}/${encodeURIComponent(collection.attributes.title)}?id=${collection.id}`}>
                                                <a title={`View collection #${collection.id}`} className='flex flex-row'>View</a>
                                            </Link>
                                        ...
                                        <div className='flex flex-col gap-2 text-left'>
                                            <div className='flex flex-row gap-4 pb-2 border-pmdGray border-b text-left'>
                                                <p><em>{collection.attributes.title}</em></p>
                                            </div>
                                            <div className='flex flex-row gap-4 text-left'>
                                                <p>Created {collection.attributes.createdAt.replace('T', ' at ').replace('Z', ' GMT')}</p>
                                            </div>
                                            <div className='flex flex-row gap-4 text-left'>
                                                <p>Updated {collection.attributes.updatedAt.replace('T', ' at ').replace('Z', ' GMT')}</p>
                                            </div>
                                            {collection.attributes.publishedAt ? (
                                                <div className='flex flex-row gap-4 text-left'>
                                                    <p>Published {collection.attributes.publishedAt.replace('T', ' at ').replace('Z', ' GMT')}</p>
                                                </div>
                                            ) : (
                                                (collection.attributes.adminReview) ? (
                                                    collection.attributes.adminReview == 'Complete' ? (
                                                        ...
                                                        <div className='flex flex-col text-left'>
                                                            <p>Under Review</p>
                                                            <p>Review Status: {collection.attributes.adminReview}</p>
                                                        </div>
                                                    )
                                                ) : (
                                                    <p>Under Review</p>
                            ...
                            <div className='flex flex-col justify-center items-center px-4 text-center'>
                                {userName ? (
                                    <div className='flex flex-col justify-center items-center gap-4 mt-6 mb-6 align-middle'>
                                        <Link href={`/${EUrlsPages.ADD_COLLECTION}`}><a
                                            title='Add New Collection'
                                            className='flex flex-row justify-center items-center gap-2 align-middle cursor-pointer'
                                        >
                                            <ImageNext
