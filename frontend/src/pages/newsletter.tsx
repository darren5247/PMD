...
const NewsletterPage: NextPage<INewsletterPageProps> = ({ prevUrl }) => {
  ...
  const { control, handleSubmit, formState, reset } = useForm<TNewsletterAddForm>();

  const handleCheckNewsletterRemoval: SubmitHandler<TNewsletterRemovalForm> = ({ email }) => {
    if (!handleError({ email })) {
      handleNewsletterRemoval(email);
    };
  };

  const handleNewsletterSubscribe: SubmitHandler<TNewsletterAddForm> = ({ name, nameLast, email }) => {
  ...
  const handleNewsletterRemoval = async (email: string) => {
    try {
      ...
      dispatch({
        type: ENotificationActionTypes.SET_MESSAGE,
        payload: {
          message: 'You have successfully removed yourself from our email newsletter',
          type: ENotificationTypes.SUCCESS
        }
      });
    } catch (error: any) {
  ...
  return (
    ...
    >
      <div className='flex flex-col justify-center items-center w-full text-center'>
        <h1>Newsletter</h1>
        <p className='mt-3 max-w-[716px] text-justify'>Sign up for our free email newsletter to receive updates from us about upcoming features, new music, deep dives into piano teaching, and more.</p>

        <h2 id='sign-up' className='mt-14'>Sign Up</h2>
        <Form>
          <div className='flex flex-col gap-y-6 pt-5 pb-16'>
            ...
            <p className='my-1 max-w-[348px] text-pmdGray text-xs text-justify'>By signing up to our email newsletter, you agree to the <Link href={`/${EUrlsPages.TERMS_AND_CONDITIONS}`}><a className='text-pmdGray' title='Terms & Conditions'>Terms & Conditions</a></Link> and <Link href={`/${EUrlsPages.PRIVACY_POLICY}`}><a className='text-pmdGray' title='Privacy Policy'>Privacy Policy</a></Link>.</p>
            <a
        ...
        <div id='manage' className='flex flex-col justify-center items-center mt-16 w-full text-center'>
          <h4 id='manage-communications'>Manage Communications</h4>
          <p className='mt-3 max-w-[716px] text-justify'>Manage your email subscription with Piano Music Database. Make the choice whether or not to receive our official email newsletter with updates and news.</p>
          <Form>
