...
import { ModalAcceptTerms } from "@src/components/Modals";
import {
...
const CreateAccountPage: NextPage<ICreateAccountPageProps> = ({ prevUrl }) => {
  ...
  useEffect(() => {
    ...
    if (accountData.id) {
      if (!accountData.name) {
        router.push(`/${EUrlsPages.ACCOUNT_SETTINGS}`, undefined, {
          shallow: false,
        });
      } else {
        router.push(`/${EUrlsPages.ACCOUNT_DASHBOARD}`, undefined, {
          shallow: false,
        });
      }
    }
  ...
  const handleCreateAccount: SubmitHandler<TCreateAccountForm> = ({
    name,
    nameLast,
    userOccupation,
    email,
  ...
  const handleData = async (form: any) => {
    try {
      ...
      router.push(
        `/${EUrlsPages.CONFIRM_CODE}?email=${encodeURIComponent(form.email)}`,
        undefined,
        { shallow: false },
      );
      reset();
      ...
      if (errorMessage?.includes("Email or Username are already taken")) {
        await api
          ...
          .then((response) => {
            if (response.data?.ok) {
              dispatch({
                ...
                payload: {
                  message:
                    "Account already exists. Confirmation email re-sent.",
                  type: ENotificationTypes.ERROR,
                },
              });
              router.push(
          ...
          .catch((err) => {
            ...
            if (alreadyConfirmed)
              router.push(
                `/${EUrlsPages.LOG_IN}?email=${encodeURIComponent(form.email)}`,
                undefined,
                { shallow: false },
              );
          });
  ...
  return (
    ...
    >
      <div className="flex flex-col text-center" id="createaccount">
        <h1>
          Welcome to <br />
          <em className="font-medium">PMD Plus</em> Early Access!
        </h1>
        <div className="flex flex-col gap-2 mx-auto my-4 sm:max-w-[640px]">
          <p className="text-sm text-justify">
            ...
            plan that is currently in development. While it is in development,
            we are opening it up for our users to try out for free! During Early
            Access, you will be able to use all of the upcoming{" "}
            <em className="font-medium">PMD Plus</em> features, including full,
            unlimited access to the search engine, favorites, and lists. Once
            the Early Access period is over, you will need to upgrade to{" "}
            <em className="font-medium">PMD Plus</em> in order to keep your data
            and continue using these features. We will let you know as Early
            Access is coming to close so that you have lots of time to make a
            decision. You will get a special discount for{" "}
            <em className="font-medium">PMD Plus</em> when it officially
            launches as a thank you for helping us test out the next generation
            of Piano Music Database!
          </p>
          <p className="text-sm text-justify">
            *<em className="font-medium">PMD Plus</em> Early Access is under
            development, and you may run into occasional bugs during usage. If
            you would like, you can report these to us, so that we are made
            aware of them and can incorporate fixes into future versions.
          </p>
          <p className="mt-4 text-base text-center">
            To get Early Access to <em className="font-medium">PMD Plus</em>,{" "}
            <br />
            create your free account by clicking <strong>Continue</strong>{" "}
            below.
          </p>
        </div>
            ...
            <div className="flex flex-col gap-y-5 mt-2">
              ...
              <div>
                <Field
                  ...
                  className="pt-[17px] pr-[19px] pb-[16px] !pl-5"
                  error={
                    formState.errors.email || findErrors(EAccountLogin.EMAIL)
                  }
                />
                <p className="text-pmdRed text-sm leading-[14px]">
              ...
              <div>
                <Field
                  ...
                  error={
                    ...
                    findErrors(EAccountLogin.PASSWORD)
                  }
                />
                <p className="text-pmdRed text-sm leading-[14px]">
