...
const ComposersPage: NextPage<IComposersPageProps> = ({ prevUrl }) => {
  ...
  const { query } = router;
  const composersBreakpoint = CItemKeys.find(item => item.value === 'composers')?.pageBreakpoint || '100000px';
  ...
  useEffect(() => {
    ...
    CFilterKeys
      .filter(({ itemType }) => itemType === 'composers')
  ...
  useEffect(() => {
    function pageTitle() {
      if (
        ...
        setPageTitleClean(`Search for ${textQuery ? `${textQuery} ` : ''}Composers${(eraFilter && eraFilter !== '') ? ` from ${decodeURIComponent(filters)}` : ''} on Piano Music Database`);
      } else {
        setFiltering(false);
    ...
    function pageDesc() {
      if (
        ...
        setPageDescClean(`Search for ${textQuery ? `${textQuery} ` : ''}Composers${(eraFilter && eraFilter !== '') ? `from ${decodeURIComponent(filters)}` : ''} on Piano Music Database. Piano Music Database is a search engine and database of pedagogical repertoire (level, composer, mood, style, and more) built for piano teachers. Find the perfect piece on PianoMusicDatabase.com.`);
      } else {
        setFiltering(false);

        setPageDescClean('Search for Composers on Piano Music Database. Piano Music Database is a search engine and database of pedagogical repertoire (level, composer, mood, style, and more) built for piano teachers. Find the perfect piece on PianoMusicDatabase.com.');
      }
    };
  ...
  return (
    <Page
      ...
      title={pageTitleClean ? pageTitleClean : 'Composers - Piano Music Database'}
      description={pageDescClean ? pageDescClean : 'Explore composers on Piano Music Database. Buy their sheet music, find the perfect works that make up their repertoire, and discover other details about your favorite composers.'}
      image=''
    ...
    >
      <div className='flex flex-col justify-center items-center text-center'>
        ...
        <p className='mx-3 mt-3 max-w-[778px] text-justify'>Explore composers on Piano Music Database. Buy their sheet music, find the perfect works that make up their repertoire, and discover other details about your favorite composers.</p>
        {userId ? (
          <div
            id='search'
            className={cn(
          ...
          >
            <SearchFiltersItem itemType='composers' className='flex' onReset={(filterName?: string) => checkFiltering(filterName)} mobileBreakpoint={composersBreakpoint} itemTypeTitleOrName='name' />
            <div
              id='searchResults'
              className={cn(
            ...
            >
              <SearchResultsItem className='flex' apiTitleOrName='name' itemType='composers' showLabel={false} pageBreakpoint={composersBreakpoint} />
            </div>
          </div>
        ) : (
          <div
...
export const getServerSideProps: GetServerSideProps<IComposersPageProps> = async (context) => {
  return {
    props: {
