...
import { IconSearchWhite, IconGlobeWhite, IconLockWhite, IconLinkWhite, IconLockRed, IconLinkRed, IconGlobeRed, IconPencilRed, IconDeleteWhite } from '@src/common/assets/icons';
...
const ListDetailsPage: NextPage<IListDetailsPageProps> = ({ listData, prevUrl }) => {
  const router = useRouter();
  const { query } = router;
  const [userId, setUserId] = useState<number | null>(null);
  const [visibilityRestriction, setVisibilityRestriction] = useState<boolean | null>(null);
  ...
  const handleIsOpenModalListDelete = () => setIsOpenModalListDelete(!isOpenModalListDelete);

  const { dispatch } = useContext(AppContext);

  let listURL: string | undefined;
  if (listData && listData[0].attributes && listData[0].attributes.title) {
    listURL = EUrlsPages.LIST + '/' + encodeURIComponent(handleTitleWithJustNumber(listData[0].attributes.title)) + '?id=' + listData[0].attributes.uid;
  } else { listURL = '' };
  ...
  useEffect(() => {
    ...
    if (accountData.id && listData) {
      ...
      if (listData[0].attributes.owners?.data?.some((owner: { id: number }) => owner.id === accountData.id)) {
        setIsOwner(true);
      }
      if (listData[0].attributes.users?.data?.some((user: { id: number }) => user.id === accountData.id)) {
        setIsUser(true);
      }
      if (listData[0].attributes.visibility.data?.attributes.currentVisibility) {
        setVisibilityValue(listData[0].attributes.visibility.data?.attributes.currentVisibility);
        if (listData[0].attributes.visibility.data?.attributes.currentVisibility == 'private') {
          if (listData[0].attributes.owners?.data?.some((owner: { id: number }) => owner.id === accountData.id) ||
            listData[0].attributes.users?.data?.some((user: { id: number }) => user.id === accountData.id)) {
      ...
      if (listData && listData[0].attributes.visibility.data?.attributes.currentVisibility == 'private') {
        setVisibilityRestriction(true);
      } else {
        setVisibilityRestriction(false);
      }
    ...
    const getListWorks = async () => {
      try {
        ...
        if (data?.meta?.pagination?.pageCount < currentPage && data?.meta?.pagination?.pageCount > 0) {
          ...
          const fetchedDataFiltered = fetchedData.map((work) => {
            try {
              ...
              return {
                ...
                listWorkId: work.id,
                owner: accountData.id ? listData[0].attributes.owners?.data?.some((owner: { id: number }) => owner.id === accountData.id) : false,
                user: accountData.id ? listData[0].attributes.users?.data?.some((user: { id: number }) => user.id === accountData.id) : false,
              };
            } catch (error) {
    ...
    getListWorks();
  }, [router, dispatch, listData, setWorks, setIsLoading, userId, currentPage, query]);
    ...
    if (listData[0].id !== null && userId !== null && newVisibility !== null) {
      try {
        const { data } = await api.put(
          `lists/${listData[0].id}`, {
          data: {
            visibility: newVisibility === 'public' ? 3 : newVisibility === 'unlisted' ? 1 : 2,
          }
        }
        );
        if (data !== null && data !== undefined) {
  ...
  const onRemove = async (listWorkId: number) => {
    if (listWorkId !== null) {
      ...
      dispatch({
        type: ENotificationActionTypes.SET_MESSAGE,
        payload: {
          message: 'Error removing piece from list, please refresh and try again.',
          type: ENotificationTypes.ERROR
        }
      });
    };
  ...
  const handleRemove = async (listWorkId: number) => {
    if (listWorkId !== null) {
      try {
        ...
        if (data !== null && data !== undefined) {
          const newWorks = works.filter(work => work.listWorkId !== listWorkId);
          setWorks(newWorks);
          dispatch({
            type: ENotificationActionTypes.SET_MESSAGE,
            payload: {
  ...
  return (
    <>
      {visibilityRestriction !== true && listData && listData[0].attributes && listData[0].attributes.title ? (
        <Page
          showBackBar={true}
          showBackBarShare={listData[0].attributes.visibility.data?.attributes.currentVisibility === 'private' ? false : true}
          showBackBarFeedback={true}
          url={listURL}
          prevUrl={prevUrl}
          title={listData[0].attributes.title ? `${listData[0].attributes.title} - Piano Music Database` : 'List - Piano Music Database'}
          description={listData[0].attributes.description ? listData[0].attributes.description : `Explore ${listData[0].attributes.title} on Piano Music Database. Buy the sheet music, find the works that make up the list, and discover other details.`}
          image=''
        >
          <div className='flex flex-col justify-center items-center w-full text-center'>
            <div className='flex flex-col justify-center items-center gap-y-3 mb-4 text-center'>
              <div className='flex flex-col mb-4 max-w-[1000px] text-center'>
                <div className='flex md:flex-row flex-col justify-center items-top max-md:items-center gap-x-6 gap-y-8'>
                  <div
                    className='flex flex-col justify-center gap-y-2 mt-4 text-center'
                  >
                    <h1
                      className='justify-center items-center pb-2 w-full max-w-[816px] overflow-ellipsis overflow-hidden text-center whitespace-normal'
                    >
                    ...
                    <div className='flex flex-col justify-center items-center gap-4 mb-2 text-center align-middle'>
                      <div className='flex flex-wrap justify-center items-center gap-x-8 gap-y-3 text-center align-middle'>
                        {listData[0].attributes.owners && listData[0].attributes.owners.data && listData[0].attributes.owners.data.length > 0 && (
                          listData[0].attributes.owners.data.map((owner: { id: number, attributes: { name: string } }, index: number) => (
                      ...
                      {listData[0].attributes.users && listData[0].attributes.users.data && listData[0].attributes.users.data.length > 0 && (
                        listData[0].attributes.users.data
                          .filter((user: { id: number }) => !listData[0].attributes.owners?.data?.some((owner: { id: number }) => owner.id === user.id))
                          .map((user: { id: number, attributes: { name: string } }, index: number) => (
                            <div key={index} className='flex flex-wrap items-center gap-2'>
                              <p className='text-pmdGray'>{user.attributes.name}</p>
                              {userId === user.id && (
                                <Link href={`/${EUrlsPages.ACCOUNT_SETTINGS}`}>
                                  <a title='Edit Account Name'>
                                    <ImageNext
                    ...
                    <div
                      className='flex min-[825px]:flex-row flex-col justify-center items-center min-[825px]:items-start gap-x-8 gap-y-2 max-w-[816px] text-center'
                    >
                      {isOwner || isUser ? (
                        <>
                          {descValue ? (
                              ...
                              {!isEditingDesc ? (
                                <a
                                  ...
                                  onClick={() => {
                                    ...
                                    setTimeout(() => {
                                      const descInput = document.getElementById(`list-desc-input-${listData[0].id}`) as HTMLTextAreaElement;
                                      if (descInput) {
                                        descInput.focus();
                                        descInput.setSelectionRange(descInput.value.length, descInput.value.length); // Set cursor at the end
                                      }
                                    }, 0);
                                  }}
                                  onKeyDown={(e) => {
                                ...
                                <div className='flex flex-col w-full h-full'>
                                  ...
                                  <div className='flex gap-2 mt-2'>
                                    <button
                                      ...
                                      onClick={() => {
                                        handleChangeDesc(descValue ? descValue : '');
                                    ...
                                    <button
                                      ...
                                      onClick={() => {
                                        setDescValue(descValueOld ? descValueOld : null);
                                        setIsEditingDesc(false);
                                      }}
                                    >
                                      ✖
                            ...
                            <div className='relative justify-start items-start w-full h-auto text-left align-middle cursor-text'>
                              {!isEditingDesc ? (
                                ...
                                >
                                  <p><em>No Description!</em> <br />Click here to add a description.</p>
                                </a>
                              ) : (
                          ...
                          <div
                            className={cn(
                              ...
                              { 'min-[825px]:text-justify': listData[0].attributes.description || (isOwner || isUser) }
                            )}
                          >
                            {listData[0].attributes.updatedAt && (<p><span className='font-medium'>Last Updated:</span> {new Date(listData[0].attributes.updatedAt).toLocaleDateString()}</p>)}
                            {listData[0].attributes.createdAt && (<p><span className='font-medium'>Created:</span> {new Date(listData[0].attributes.createdAt).toLocaleDateString()}</p>)}
                            {isOwner || (isOwner && isUser) ? (
                              <div className='flex flex-col max-[825px]:justify-center max-[825px]:items-center gap-2 max-[825px]:text-center max-[825px]:align-middle'>
                                <a
                                  ...
                                  aria-controls={'ModalListDelete' + listData[0].id}
                                ...
                                >
                                  <ImageNext src={IconDeleteWhite} alt='' height={16} width={16} />
                            ...
                            {visibilityValue && (
                              <div className='relative flex flex-col max-[825px]:justify-center max-[825px]:items-center max-[825px]:text-center'>
                                <p><span className='font-medium'>List Visibility:</span></p>
                                <div className='flex flex-col max-[825px]:justify-center max-[825px]:items-center gap-2 max-[825px]:text-center max-[825px]:align-middle'>
                                  <a
                                  ...
                                  >
                                    {visibilityValue === 'public' ? (
                                      <div className='flex gap-2'>
                                        <ImageNext src={IconGlobeWhite} alt='' height={28} width={28} />
                                        <p>Public</p>
                                      </div>
                                    ) : visibilityValue === 'unlisted' ? (
                                      <div className='flex gap-2'>
                                        <ImageNext src={IconLinkWhite} alt='' height={28} width={28} />
                                        <p>Unlisted</p>
                                      </div>
                                    ) : (
                                      <div className='flex gap-2'>
                                        <ImageNext src={IconLockWhite} alt='' height={28} width={28} />
                                        <p>Private</p>
                                      </div>
                                    )}
                                  </a>
                                </div>
                                <div id='visibility-dropdown' className='hidden top-20 z-50 absolute shadow-lg border rounded text-center align-middle'>
                                  <button
                                  ...
                                  >
                                    <ImageNext src={IconGlobeRed} alt='' height={28} width={28} />
                                    <p>Public</p>
                                  </button>
                                  <button
                                    className='flex justify-center gap-2 bg-white hover:bg-pmdGrayBright px-4 py-2 w-full text-pmdRed hover:text-pmdGray text-center align-middle'
                                  ...
                                  >
                                    <ImageNext src={IconLinkRed} alt='' height={28} width={28} />
                                    <p>Unlisted</p>
                                  </button>
                                  <button
                                    className='flex justify-center gap-2 bg-white hover:bg-pmdGrayBright px-4 py-2 w-full text-pmdRed hover:text-pmdGray text-center align-middle'
                                  ...
                                  >
                                    <ImageNext src={IconLockRed} alt='' height={28} width={28} />
                                    <p>Private</p>
                                  </button>
                                </div>
                              </div>
                ...
                {detailsValue ? (
                    ...
                    {!isEditingDetails ? (
                      <div className='flex flex-col justify-center gap-y-2 mt-4 text-center'>
                        <div className='relative w-full max-w-[816px]'>
                          <div className='flex gap-2 mt-2 mb-0 ml-2 w-full text-left'>
                            ...
                            <a
                              ...
                              onClick={() => {
                                ...
                                setTimeout(() => {
                                  const detailsInput = document.getElementById(`list-details-input-${listData[0].id}`) as HTMLTextAreaElement;
                                  if (detailsInput) {
                                    detailsInput.focus();
                                    detailsInput.setSelectionRange(detailsInput.value.length, detailsInput.value.length); // Set cursor at the end
                                  }
                                }, 0);
                              }}
                              onKeyDown={(e) => {
                          ...
                          <div className='px-3 py-2 border border-pmgGray rounded-md max-h-40 overflow-y transition-max-height duration-300 ease-in-out' id='content-frame'>
                          ...
                          <button
                            ...
                            onClick={() => {
                              ...
                              if (frame) {
                                ...
                                const button = frame.nextElementSibling as HTMLButtonElement;
                                if (button) {
                                  button.textContent = frame.classList.contains('max-h-96') ? 'Collapse' : 'Expand';
                                }
                              }
                            }}
                          >
                      ...
                      <div className='flex flex-col w-full h-full'>
                        ...
                        <div className='flex gap-2 mt-2 w-full h-full grow'>
                          <button
                            className='bg-green-500 p-1 rounded w-[25px] h-[25px] text-white'
                            onClick={() => {
                              handleChangeDetails(detailsValue ? detailsValue : '');
                          ...
                          <button
                            className='bg-red-500 p-1 rounded w-[25px] h-[25px] text-white'
                            onClick={() => {
                              setDetailsValue(detailsValueOld ? detailsValueOld : null);
                              setIsEditingDetails(false);
                            }}
                          >
                            ✖
                  ...
                  <div className='relative justify-start items-start w-full h-full text-left align-middle cursor-text'>
                    {!isEditingDetails ? (
                      ...
                      >
                        <p><em>No Details!</em> <br />Click here to add details.</p>
                      </a>
                    ) : (
                      <div className='flex flex-col w-full h-full'>
                        <div className='flex gap-2 mt-2 mb-0 ml-2 w-full text-left'>
            ...
            <div id='list-works' className='flex flex-col flex-wrap justify-center items-center mt-10 w-full'>
              {isLoading ? (
                <p>Loading works...</p>
              ) : (
                works && works.length > 0 ? (
                  <div className='relative w-full max-w-[816px]'>
                    <Works works={works} onOrderChange={handleOrderChange} onRemoveListItem={onRemove} />
                  </div>
                ) : (
                  <div className='relative flex flex-col justify-center gap-y-2 mx-auto w-auto max-w-[816px] text-center'>
                    <Chip title='This list is currently empty!' className='flex justify-center items-center !text-red-600 text-center align-middle' />
                    <Chip title='Find and add some pieces to this list.' className='flex justify-center items-center text-center align-middle' />
                  </div>
                )
              )}
            </div>
        ...
        visibilityRestriction == true ? (
          ...
          >
            <>
              ...
              <h2 className='mt-10'><em>This list has a private visibility.</em></h2>
              <p className='mt-6 max-w-[864px]'>The list you are looking for has been set to private visibility.</p>
              <p className='mt-2 max-w-[864px]'>If you are an owner or member of this list, try logging into the correct account associated with this list.</p>
              <p className='mt-2 max-w-[844px]'>If you were looking for a specific list, make sure the URL is formatted like this: <br /><strong>/{EUrlsPages.LIST}/TITLEHERE?id=IDHERE</strong> <br />The ID is a number we use to connect you to the correct list. Some lists may have the same title, so we can not know which list you are looking for without an ID in the URL.</p>
          ...
          (
            ...
            >
              <>
                ...
                <h2 className='mt-10'><em>This list was not able to be located.</em></h2>
                <p className='mt-6 max-w-[864px]'>The list you are looking for has moved, is no longer available, has been archived, or was not valid.</p>
                <p className='mt-2 max-w-[844px]'>If you were looking for a specific list, make sure the URL is formatted like this: <br /><strong>/{EUrlsPages.LIST}/TITLEHERE?id=IDHERE</strong> <br />The ID is a number we use to connect you to the correct list. Some lists may have the same title, so we can not know which list you are looking for without an ID in the URL.</p>
                <p className='mt-6'>Try searching to find what you are looking for.</p>
                <div className='flex flex-col justify-center items-center gap-y-4 my-8 w-full'>
                  <Link href={`/${EUrlsPages.SEARCH}`}>
