...
interface ILogInConfirmedForm {
  [EAccountLogin.EMAIL]: string;
  [EAccountLogin.PASSWORD]: string;
};

type TLogInConfirmedForm = ILogInConfirmedForm | FieldValues;

interface ILogInConfirmedPageProps {
  prevUrl: string | undefined;
};

const LogInConfirmedPage: NextPage<ILogInConfirmedPageProps> = ({ prevUrl }) => {
  const router = useRouter();
  const { dispatch } = useContext(AppContext);
  const [errors, setErrors] = useState<IError[]>([]);
  const { control, handleSubmit, formState, reset } = useForm<TLogInConfirmedForm>();
  ...
  const handleLogInConfirmed: SubmitHandler<TLogInConfirmedForm> = ({ email, password }) => {
  ...
  const handleData = async (email: string, password: string) => {
    try {
      ...
      if (jwt) {
        document.cookie = `jwt=${jwt}; max-age=2592000;`;
  ...
  return (
    <Page
      ...
      url={EUrlsPages.LOG_IN + '-confirmed'}
      prevUrl={prevUrl}
    ...
    >
      <div className='flex flex-col sm:max-w-[340px]' id='login'>
        <h1>Log In</h1>
        <p className='bg-pmdGrayLight mt-4 p-3 rounded'>Email Confirmed! <br />Please Log In to continue.</p>
        <Form onSubmit={handleSubmit(handleLogInConfirmed)}>
