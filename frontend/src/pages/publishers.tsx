...
const PublishersPage: NextPage<IPublishersPageProps> = ({ prevUrl }) => {
  ...
  const { query } = router;
  const publishersBreakpoint =
    CItemKeys.find((item) => item.value === "publishers")?.pageBreakpoint ||
    "100000px";
  ...
  useEffect(() => {
    ...
    CFilterKeys.filter(({ itemType }) => itemType === "publishers").forEach(
      ({ filterOptions }) => {
  ...
  useEffect(() => {
    function pageTitle() {
      if ((textQuery && textQuery !== "") || (eraFilter && eraFilter !== "")) {
        ...
        setPageTitleClean(
          `Search for ${textQuery ? `${textQuery} ` : ""}Publishers${eraFilter && eraFilter !== "" ? ` from ${decodeURIComponent(filters)}` : ""} on Piano Music Database`,
        );
      } else {
    ...
    function pageDesc() {
      if ((textQuery && textQuery !== "") || (eraFilter && eraFilter !== "")) {
        ...
        setPageDescClean(
          `Search for ${textQuery ? `${textQuery} ` : ""}Publishers${eraFilter && eraFilter !== "" ? `from ${decodeURIComponent(filters)}` : ""} on Piano Music Database. Piano Music Database is a search engine and database of pedagogical repertoire (level, publisher, mood, style, and more) built for piano teachers. Find the perfect piece on PianoMusicDatabase.com.`,
        );
      } else {
        setPageDescClean(
          "Search for Publishers on Piano Music Database. Piano Music Database is a search engine and database of pedagogical repertoire (level, publisher, mood, style, and more) built for piano teachers. Find the perfect piece on PianoMusicDatabase.com.",
        );
      }
    }
  ...
  return (
    <Page
      ...
      title={
        pageTitleClean ? pageTitleClean : "Publishers - Piano Music Database"
      }
      description={
        pageDescClean
          ? pageDescClean
          : "Explore publishers on Piano Music Database. Buy their sheet music, find the perfect works that make up their repertoire, and discover other details about your favorite publishers."
      }
      image=""
    ...
    >
      <div className="flex flex-col justify-center items-center text-center">
        ...
        <p className="mx-3 mt-3 max-w-[778px] text-justify">
          Explore publishers on Piano Music Database. Buy their sheet music,
          find the perfect works that make up their repertoire, and discover
          other details about your favorite publishers.
        </p>
        {userId ? (
          <div
            id="search"
          ...
          >
            <SearchFiltersItem
              ...
              mobileBreakpoint={publishersBreakpoint}
            ...
            >
              <SearchResultsItem
                ...
                itemType="publishers"
                showLabel={false}
                pageBreakpoint={publishersBreakpoint}
              />
            </div>
          </div>
        ) : (
