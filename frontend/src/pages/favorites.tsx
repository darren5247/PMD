  ...
  const onRemove = async (favoritedWorkId: number, favoriteId: number) => {
    if (favoriteId !== null) {
      ...
      dispatch({
        ...
        payload: {
          message:
            "Error removing piece from favorites, please refresh and try again.",
          type: ENotificationTypes.ERROR,
        },
      });
    }
  ...
  return (
    ...
    >
      <div className="flex flex-col justify-center items-center text-center favorites-center">
        <h1>Favorites</h1>
        <p className="mt-3 max-w-[570px]">
          Add pieces to your favorites to quickly save those pieces for later.{" "}
          <br />
          Simply click the heart beside a work to favorite or unfavorite it.{" "}
          <br />
          Your favorite works can also be added to lists, for further
          organization.
        </p>
        <div
          id="favorites"
          className="flex flex-col flex-wrap justify-center mt-8 w-full max-w-[1100px] favorites-center"
        >
          {isLoadingFavorites ? (
            ...
            <div className="flex flex-row flex-wrap justify-center items-stretch gap-3 my-4 max-w-[816px] text-center align-middle favorites-center">
              ...
              {favorites.length > 9 && (
                <div className="flex justify-center items-center gap-3 min-[340px]:gap-5 mt-6 mb-3 text-sm">
                  ...
                  <Link
                    href={"/" + EUrlsPages.FAVORITES + ("?page=" + totalPages)}
                  >
                    <a
                      title="Last Page"
                      className={cn(
