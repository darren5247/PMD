...
const AccountSettingsPage: NextPage<IAccountSettingsPageProps> = ({
  prevUrl,
}) => {
  ...
  const [isOpenModalNewsletterAccount, setIsOpenModalNewsletterAccount] =
    useState(false);
  const handleIsOpenModalNewsletterAccount = () =>
    setIsOpenModalNewsletterAccount(!isOpenModalNewsletterAccount);
  ...
  useEffect(() => {
    ...
    const checkSubscribed = async (email: string) => {
      try {
        ...
        setSubscribed(
          [8, 28, 29, 30, 31].some((id) => data.data.listIds.includes(id)),
        );
      } catch (error: any) {
        if (error?.response?.data) {
          console.error("Error checking subscription:", error?.response?.data);
        }
      }
    };
  ...
  const handleChangeDetailsSettings: SubmitHandler<TSettingsForm> = ({
    name,
    nameLast,
    email,
    username,
  ...
  const handleError = (form: any) => {
    ...
    for (const key in accountSettingsRules) {
      if (!accountSettingsRules[key].rule(form[key]))
        newErrors.push({
          name: key,
          message: accountSettingsRules[key].message,
        });
    }
      ...
      <Form>
        <div className="flex flex-col sm:max-w-[340px]">
          ...
          <div className="flex flex-col gap-y-6 pt-3 pb-8">
            ...
            <div>
              <Field
                labelEl={<Label htmlFor={EAccountData.email} label="Email" />}
                labelClassName="font-bold mb-1"
              ...
              <p className="pt-1 text-pmdGray text-sm italic">
                <Link href={`/${EUrlsPages.CONTACT}`}>
                  <a title="Contact us" className="text-pmdGray">
                    Contact us
                  </a>
                </Link>{" "}
