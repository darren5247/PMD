...
const WorkDetailsPage: NextPage<IWorkDetailsPageProps> = ({
  ...
  useEffect(() => {
    ...
    if (router.query.id && router.query.id === "381") {
      router.push(
        `/${EUrlsPages.WORK}/A%20Little%20Study-Ben%20Crosland?id=2751`,
        undefined,
        { shallow: false },
      );
    }
    ...
    <>
      {musicWork && musicWork.attributes.title ? (
        <Page
          ...
          image={
            musicWork.attributes.imageSEO?.data
              ? musicWork.attributes.imageSEO.data?.attributes.url
              : ""
          }
          className={cn({
            "!max-w-full !mx-0 !px-0": musicWork.attributes.promoText,
          })}
          classNameMain={cn({
            "!mt-[35px] lg:!mt-[35px] !max-w-full !mx-0 !px-0":
        ...
        >
          <>
            ...
            <h2 className="mt-10">
              <em>This piece was not able to be located.</em>
            </h2>
            <p className="mt-6 max-w-[810px]">
              The piece you are looking for has moved, is no longer available,
              has been archived, or was not valid.
            </p>
            <p className="mt-2 max-w-[808px]">
              If you were looking for a specific piece, make sure the URL is
              formatted like this: <br />
              <strong>/{EUrlsPages.WORK}/TITLEHERE?id=IDHERE</strong> <br />
              The ID is a number we use to connect you to the correct piece.
              Some pieces may have the same title, so we can not know which
              piece you are looking for without an ID in the URL.
            </p>
            <p className="mt-6">
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  try {
    const response = await apiPiece.get((context?.query?.id as string) || "0");
    return {
      props: {
