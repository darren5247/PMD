...
<urlset xmlns='http://www.sitemaps.org/schemas/sitemap/0.9' xmlns:xhtml='http://www.w3.org/1999/xhtml' xmlns:mobile='http://www.google.com/schemas/sitemap-mobile/1.0'>` + data + `
   </urlset>
 `;
};
...
function generateWorksXML() {
  ...
  works.forEach(work => {
    ...
    if (work.attributes.title) {
      ...
      workTitleEncoded = encodeURIComponent(titleSaved) + '-' + encodeURIComponent(workComposers);
      workURL = siteUrl + '/' + EUrlsPages.WORK + '/' + workTitleEncoded + '?id=' + work.id;
    }
    else { titleSaved = ''; workTitleEncoded = ''; workURL = '' };
