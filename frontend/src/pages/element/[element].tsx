...
import { IconSearchRed, IconSearchWhite, IconFeedbackWhite } from '@src/common/assets/icons';
...
const ElementDetailsPage: NextPage<IElementDetailsPageProps> = ({ musicElement, prevUrl }) => {
  ...
  if (musicElement && musicElement.attributes.name) {
    elementURL = EUrlsPages.ELEMENT + '/' + encodeURIComponent(checkNumberName(musicElement.attributes.name)) + '?id=' + musicElement.id;
  } else { elementURL = '' };

  let elementID: number | undefined;
  if (musicElement && musicElement.id) {
    elementID = musicElement.id;
  } else { elementID = undefined };
  ...
  useEffect(
        ...
        const getPieces = async () => {
          try {
            ...
            if (data?.meta?.pagination?.pageCount < currentPagePieces && data?.meta?.pagination?.pageCount > 0) {
              ...
              router.push({
                pathname: encodeURIComponent(handleTitleWithJustNumber(musicElement.attributes.name)),
                query: pagePieces ? { id: musicElement.id, pageCollections: pagePieces, pagePieces: data.meta.pagination.pageCount } : { id: musicElement.id, pagePieces: data.meta.pagination.pageCount }
              });
            } else {
            ...
            if (error?.response?.data) {
              console.error('Error Getting Pieces for Composer-' + elementID + ': ' + error?.response?.data.error?.message);
            };
          } finally {
    ...
    }, [elementID, dispatch, router, currentPagePieces, query, setPieces, setIsLoadingPieces, musicElement?.id, musicElement?.attributes?.name]
  );
    ...
    <>
      {musicElement && musicElement.attributes.name ? (
        <Page
          ...
          url={elementURL}
          prevUrl={prevUrl}
          title={musicElement.attributes.name ? `${musicElement.attributes.name} - Piano Music Database` : 'Element - Piano Music Database'}
          description={musicElement.attributes.description ? musicElement.attributes.description : `Explore ${musicElement.attributes.name} on Piano Music Database. Elements are the fundamental pedagogical concepts and techniques contained in each piece of piano music.`}
          image=''
        >
            ...
            {!musicElement.attributes.publishedAt && (
              <section id='review' className='flex justify-center'>
                <div className='bg-orange-500 shadow-workNav mx-4 mb-12 py-4 rounded-lg'>
                  ...
                  <p className='px-4 text-white text-center tracking-thigh'><strong>This element is currently <em>under review</em> by staff.</strong></p>
                  <p className='mt-4 px-4 text-white text-center tracking-thigh'><strong>This page is publicly viewable while under review, but the element will not appear in PMD search results.</strong></p>
                ...
                {musicElement.attributes.illustration.data ? (
                  <div className='p-3 min-w-auto sm:min-w-[300px] max-w-[240px] sm:max-w-[300px]'>
                    <ImagePicture
                      ...
                      alt={musicElement.attributes.illustration.data?.attributes.alternativeText}
                      src={musicElement.attributes.illustration.data?.attributes.url ? musicElement.attributes.illustration.data?.attributes.url : ''}
                      height={musicElement.attributes.illustration.data?.attributes.height ? musicElement.attributes.illustration.data?.attributes.height : 0}
                      width={musicElement.attributes.illustration.data?.attributes.width ? musicElement.attributes.illustration.data?.attributes.width : 0}
                    />
                  </div>
                ) : ''}
                <div
                  className={cn(
                    'mt-4 flex gap-y-4 flex-col text-center',
                    {
                      'md:text-left': musicElement.attributes.illustration.data
                    }
                  )}
                >
                  {musicElement.attributes.name ? (<h1
                    className={cn(
                      {
                        'pb-2': !musicElement.attributes.illustration.data
                      }
                    )}>{musicElement.attributes.name}</h1>) : ''}
                  {musicElement.attributes.category ? (<p><strong><em>{musicElement.attributes.category}</em></strong></p>) : ''}
                  {musicElement.attributes.description ? (<div
                    className='md:w-full md:max-w-[816px] text-justify'
                    dangerouslySetInnerHTML={
                      {
                        __html: handleCleanContent(musicElement.attributes.description)
                      }
                    }
                  />) : ''}
                </div>
                ...
                <div className='flex flex-col justify-center items-center gap-1 mt-10 text-center align-middle'>
                  ...
                  {pieces.length > 9 && (
                    <div className='flex justify-center items-center gap-3 min-[340px]:gap-5 mt-6 mb-3 text-sm'>
                      <Link
                        href={(
                          encodeURIComponent(handleTitleWithJustNumber(musicElement.attributes.name)) +
                      ...
                      <Link
                        href={(
                          encodeURIComponent(handleTitleWithJustNumber(musicElement.attributes.name)) +
                          ('?id=' + musicElement.id + '&page=' + (currentPagePieces - 1))
                          + '#Pieces'
                        )}
                      >
                        <a
                      ...
                      <Link
                        href={(
                          encodeURIComponent(handleTitleWithJustNumber(musicElement.attributes.name)) +
                          ('?id=' + musicElement.id + '&page=' + totalPagesPieces)
                          + '#Pieces'
                        )}
                      >
                        <a
        ...
        (
          ...
          >
            <>
              ...
              <h2 className='mt-10'><em>This element was not able to be located.</em></h2>
              <p className='mt-6 max-w-[846px]'>The element you are looking for has moved, is no longer available, has been archived, or was not valid.</p>
              <p className='mt-2 max-w-[844px]'>If you were looking for a specific element, make sure the URL is formatted like this: <br /><strong>/{EUrlsPages.ELEMENT}/NAMEHERE?id=IDHERE</strong> <br />The ID is a number we use to connect you to the correct element. Some elements may have the same name, so we can not know which element you are looking for without an ID in the URL.</p>
