...
const PublisherDetailsPage: NextPage<IPublisherDetailsPageProps> = ({
  musicPublisher,
  prevUrl,
}) => {
  ...
  if (musicPublisher && musicPublisher.attributes.name) {
    publisherURL =
      EUrlsPages.PUBLISHER +
      "/" +
      encodeURIComponent(checkNumberName(musicPublisher.attributes.name)) +
      "?id=" +
      musicPublisher.id;
  } else {
    publisherURL = "";
  ...
  useEffect(() => {
    ...
    if (router.query.id && router.query.id === "1") {
      router.push(`/${EUrlsPages.PUBLISHER}/Alfred Music?id=82`, undefined, {
        shallow: false,
      });
    }
    ...
    if (publisherID) {
      const getCollections = async () => {
        try {
          ...
          if (
            ...
            router.push({
              pathname: encodeURIComponent(
                handleTitleWithJustNumber(musicPublisher.attributes.name),
              ),
              query: pagePieces
                ? {
                    id: musicPublisher.id,
          ...
          if (error?.response?.data) {
            console.error(
              "Error Getting Collections for Publisher-" +
                publisherID +
                ": " +
      ...
      const getPieces = async () => {
        try {
          ...
          if (error?.response?.data) {
            console.error(
              "Error Getting Pieces for Publisher-" +
                publisherID +
                ": " +
                error?.response?.data.error?.message,
            );
    ...
    <>
      {musicPublisher && musicPublisher.attributes.name ? (
        <Page
          ...
          title={
            musicPublisher.attributes.name
              ? `${musicPublisher.attributes.name} - Piano Music Database`
              : "Publisher - Piano Music Database"
          }
          description={
            musicPublisher.attributes.excerpt
              ? musicPublisher.attributes.excerpt
              : `Explore ${musicPublisher.attributes.name} on Piano Music Database.`
          }
          image={
            musicPublisher.attributes.imageSEO?.data
              ? musicPublisher.attributes.imageSEO.data?.attributes.url
              : ""
          }
              ...
              <section id="review" className="flex justify-center">
                <div className="bg-orange-500 shadow-workNav mx-4 mb-12 py-4 rounded-lg">
                  ...
                  <p className="px-4 text-white text-center tracking-thigh">
                    <strong>
                      This publisher is currently <em>under review</em> by
                      staff.
                    </strong>
                  </p>
                  <p className="mt-4 px-4 text-white text-center tracking-thigh">
                    <strong>
                      ...
                      publisher will not appear in PMD search results.
                    </strong>
                  </p>
                  <p className="mt-4 px-4 text-white text-center tracking-thigh">
                    <strong>
                ...
                <div className="flex min-[935px]:flex-row flex-col justify-center items-top max-[935px]:items-center gap-x-6 gap-y-8">
                  {musicPublisher.attributes.image.data ? (
                    <div className="min-[935px]:mt-5 min-w-auto sm:min-w-[300px] max-w-[240px] sm:max-w-[300px]">
                      <ImagePicture
                        alt={
                          musicPublisher.attributes.image.data?.attributes
                            .alternativeText
                            ? musicPublisher.attributes.image.data?.attributes
                                .alternativeText
                            : musicPublisher.attributes.name
                        ...
                        src={
                          musicPublisher.attributes.image.data?.attributes.url
                            ? musicPublisher.attributes.image.data?.attributes
                                .url
                            : ""
                        }
                        ...
                        width={
                          musicPublisher.attributes.image.data?.attributes.width
                            ? musicPublisher.attributes.image.data?.attributes
                                .width
                            : 0
                        }
                  ...
                  >
                    ...
                    {musicPublisher.attributes.nationality ? (
                      <p>
                        <em>{musicPublisher.attributes.nationality}</em>
                      </p>
                    ) : (
                      ""
                    )}
                    ...
                    <div className="flex flex-row flex-wrap justify-center items-center gap-x-2 gap-y-1 shadow-musicCard p-1 rounded-lg text-center">
                      <Link href={musicPublisher.attributes.urlSpotify}>
                        <a
                          ...
                          title={`Listen to ${musicPublisher.attributes.name} on Spotify`}
                          target="_blank"
                  ...
                  {musicPublisher.attributes.urlAppleMusic && (
                    <div className="flex flex-row flex-wrap justify-center items-center gap-x-2 gap-y-1 shadow-musicCard p-1 rounded-lg text-center">
                      <Link href={musicPublisher.attributes.urlAppleMusic}>
                        <a
                          className="group flex justify-center items-center text-center"
                          title={`Listen to ${musicPublisher.attributes.name} on Apple Music`}
                          target="_blank"
                          rel="noopener noreferrer"
                        >
                          <ImageNext
                ...
                <div className="flex flex-row flex-wrap justify-center items-center gap-x-2 gap-y-1 shadow-musicCard p-3 rounded-lg text-center">
                  <Link href={musicPublisher.attributes.urlWebsite}>
                    <a
                      ...
                      title={musicPublisher.attributes.urlWebsite}
                      target="_blank"
                      rel="noopener noreferrer"
                    >
                ...
                <div className="flex flex-wrap justify-center items-center gap-2 max-md:mt-2 text-center">
                  {musicPublisher.attributes.urlSocialInstagram && (
                    <Link href={musicPublisher.attributes.urlSocialInstagram}>
                      <a
                        className="group flex justify-center items-center text-center"
                        title={`${musicPublisher.attributes.name} on Instagram`}
                        target="_blank"
                        rel="noopener noreferrer"
                      >
                        <ImageNext
                  ...
                  {musicPublisher.attributes.urlSocialFacebook && (
                    <Link href={musicPublisher.attributes.urlSocialFacebook}>
                      <a
                        className="group flex justify-center items-center text-center"
                        title={`${musicPublisher.attributes.name} on Facebook`}
                        target="_blank"
                        rel="noopener noreferrer"
                      >
                        <ImageNext
                  ...
                  {musicPublisher.attributes.urlSocialX && (
                    <Link href={musicPublisher.attributes.urlSocialX}>
                      <a
                        className="group flex justify-center items-center text-center"
                        title={`${musicPublisher.attributes.name} on X/Twitter`}
                        target="_blank"
                        rel="noopener noreferrer"
                      >
                        <ImageNext
                  ...
                  {musicPublisher.attributes.urlSocialLinkedIn && (
                    <Link href={musicPublisher.attributes.urlSocialLinkedIn}>
                      <a
                        className="group flex justify-center items-center text-center"
                        title={`${musicPublisher.attributes.name} on LinkedIn`}
                        target="_blank"
                        rel="noopener noreferrer"
                      >
                        <ImageNext
                  ...
                  {musicPublisher.attributes.urlSocialYouTube && (
                    <Link href={musicPublisher.attributes.urlSocialYouTube}>
                      <a
                        className="group flex justify-center items-center text-center"
                        title={`${musicPublisher.attributes.name} on YouTube`}
                        target="_blank"
                        rel="noopener noreferrer"
                      >
                        <ImageNext
                      ...
                      href={
                        ...
                        (query.pageCollections
                          ? "?id=" +
                            ...
                            "&pageCollections=" +
                            query.pageCollections
                          : "?id=" + musicPublisher.id + "&pagePieces=1") +
                        "#Pieces"
                      }
                    >
                      <a
        ...
        >
          <>
            ...
            <h2 className="mt-10">
              <em>This publisher was not able to be located.</em>
            </h2>
            <p className="mt-6 max-w-[842px]">
              The publisher you are looking for has moved, is no longer
              available, has been archived, or was not valid.
            </p>
            <p className="mt-2 max-w-[840px]">
              If you were looking for a specific publisher, make sure the URL is
              formatted like this: <br />
              <strong>/{EUrlsPages.PUBLISHER}/NAMEHERE?id=IDHERE</strong> <br />
              The ID is a number we use to connect you to the correct publisher.
              Some publishers may have the same name, so we can not know which
              publisher you are looking for without an ID in the URL.
            </p>
            <p className="mt-6">
