...
const PublishersAddedPage: NextPage<IPublishersAddedPageProps> = ({
  prevUrl,
}) => {
  ...
  return (
    ...
    >
      <div className="max-w-[460px] text-center">
        <h1 className="mx-auto">Publishers Added</h1>
        <>
          {isLoadingPublishers ? (
            ...
            <div>
              ...
              {publishers.map((publisher) => (
                ...
                >
                  <div className="flex flex-col gap-4 text-left">
                    ...
                    <Link
                      href={`/${EUrlsPages.PUBLISHER}/${encodeURIComponent(publisher.attributes.name)}?id=${publisher.id}`}
                    >
                      <a
                        title={`View publisher #${publisher.id}`}
                  ...
                  <div className="flex flex-col gap-2 text-left">
                    <div className="flex flex-row gap-4 pb-2 border-pmdGray border-b text-left">
                      <p>
                        <em>{publisher.attributes.name}</em>
                      </p>
                    </div>
                    <div className="flex flex-row gap-4 text-left">
                      <p>
                        Created{" "}
                        {publisher.attributes.createdAt
                          .replace("T", " at ")
                          .replace("Z", " GMT")}
                      </p>
                    </div>
                    <div className="flex flex-row gap-4 text-left">
                      <p>
                        Updated{" "}
                        {publisher.attributes.updatedAt
                          .replace("T", " at ")
                          .replace("Z", " GMT")}
                      </p>
                    </div>
                    {publisher.attributes.publishedAt ? (
                      ...
                      publisher.attributes.adminReview == "Complete" ? (
                        ...
                        <div className="flex flex-col text-left">
                          <p>Under Review</p>
                          <p>
                            Review Status: {publisher.attributes.adminReview}
                          </p>
                        </div>
                      )
                    ) : (
              ...
              {publishers.length > 9 && (
                <div className="flex justify-center items-center gap-3 min-[340px]:gap-5 mt-6 mb-3 text-sm">
                  <Link
                    href={EUrlsPages.PUBLISHERS_ADDED + "?page=1#worksAdded"}
                  >
                    <a
                      title="First Page"
                      className={cn(
