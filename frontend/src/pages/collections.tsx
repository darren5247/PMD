...
const CollectionsPage: NextPage<ICollectionsPageProps> = ({ prevUrl }) => {
  ...
  const { query } = router;
  const collectionsBreakpoint =
    CItemKeys.find((item) => item.value === "collections")?.pageBreakpoint ||
    "100000px";
  ...
  useEffect(() => {
    ...
    CFilterKeys.filter(({ itemType }) => itemType === "collections").forEach(
      ({ filterOptions }) => {
  ...
  useEffect(() => {
    function pageTitle() {
      if ((textQuery && textQuery !== "") || (eraFilter && eraFilter !== "")) {
        ...
        setPageTitleClean(
          `Search for ${textQuery ? `${textQuery} ` : ""}Collections${eraFilter && eraFilter !== "" ? ` from ${decodeURIComponent(filters)}` : ""} on Piano Music Database`,
        );
      } else {
    ...
    function pageDesc() {
      if ((textQuery && textQuery !== "") || (eraFilter && eraFilter !== "")) {
        ...
        setPageDescClean(
          `Search for ${textQuery ? `${textQuery} ` : ""}Collections${eraFilter && eraFilter !== "" ? `from ${decodeURIComponent(filters)}` : ""} on Piano Music Database. Piano Music Database is a search engine and database of pedagogical repertoire (level, collection, mood, style, and more) built for piano teachers. Find the perfect piece on PianoMusicDatabase.com.`,
        );
      } else {
        setPageDescClean(
          "Search for Collections on Piano Music Database. Piano Music Database is a search engine and database of pedagogical repertoire (level, collection, mood, style, and more) built for piano teachers. Find the perfect piece on PianoMusicDatabase.com.",
        );
      }
    }
  ...
  return (
    <Page
      ...
      title={
        pageTitleClean ? pageTitleClean : "Collections - Piano Music Database"
      }
      description={
        pageDescClean
          ? pageDescClean
          : "Explore collections on Piano Music Database. Buy their sheet music, find the perfect works that make up their repertoire, and discover other details about your favorite collections."
      }
      image=""
    ...
    >
      <div className="flex flex-col justify-center items-center text-center">
        ...
        <p className="mx-3 mt-3 max-w-[778px] text-justify">
          Explore collections on Piano Music Database. Buy their sheet music,
          find the perfect works that make up their repertoire, and discover
          other details about your favorite collections.
        </p>
        {userId ? (
          <div
            id="search"
          ...
          >
            <SearchFiltersItem
              ...
              mobileBreakpoint={collectionsBreakpoint}
              itemTypeTitleOrName="title"
            />
            <div
              id="searchResults"
              className={cn(
            ...
            >
              <SearchResultsItem
                ...
                itemType="collections"
                showLabel={false}
                pageBreakpoint={collectionsBreakpoint}
              />
            </div>
          </div>
        ) : (
