...
const CollectionsPage: NextPage<ICollectionsPageProps> = ({ prevUrl }) => {
  ...
  const { query } = router;
  const collectionsBreakpoint = CItemKeys.find(item => item.value === 'collections')?.pageBreakpoint || '100000px';
  const isBreakpoint = useMediaQuery(`(min-width: ${collectionsBreakpoint})`);
  ...
  useEffect(() => {
    ...
    CFilterKeys
      .filter(({ itemType }) => itemType === 'collections')
  ...
  useEffect(() => {
    function pageTitle() {
      if (
        ...
        setPageTitleClean(`Search for ${textQuery ? `${textQuery} ` : ''}Collections${(eraFilter && eraFilter !== '') ? ` from ${decodeURIComponent(filters)}` : ''} on Piano Music Database`);
      } else {
        setFiltering(false);

        setPageTitleClean('Search for Collections on Piano Music Database');
      }
    };
    ...
    function pageDesc() {
      if (
        ...
        setPageDescClean(`Search for ${textQuery ? `${textQuery} ` : ''}Collections${(eraFilter && eraFilter !== '') ? `from ${decodeURIComponent(filters)}` : ''} on Piano Music Database. Piano Music Database is a search engine and database of pedagogical repertoire (level, collection, mood, style, and more) built for piano teachers. Find the perfect piece on PianoMusicDatabase.com.`);
      } else {
        setFiltering(false);

        setPageDescClean('Search for Collections on Piano Music Database. Piano Music Database is a search engine and database of pedagogical repertoire (level, collection, mood, style, and more) built for piano teachers. Find the perfect piece on PianoMusicDatabase.com.');
      }
    };
  ...
  return (
    <Page
      ...
      title={pageTitleClean ? pageTitleClean : 'Collections - Piano Music Database'}
      description={pageDescClean ? pageDescClean : 'Explore collections on Piano Music Database. Buy their sheet music, find the perfect works that make up their repertoire, and discover other details about your favorite collections.'}
      image=''
      className='!mx-0 !px-0 !max-w-full'
      classNameMain='!max-w-full !mx-0 !px-0'
    >
      <div className='flex flex-col justify-center items-center text-center'>
        <h1 className='mx-3'>Collections</h1>
        <p className='mx-3 mt-3 max-w-[778px] text-justify'>Explore collections on Piano Music Database. Buy their sheet music, find the perfect works that make up their repertoire, and discover other details about your favorite collections.</p>
        {userId ? (
          <div
            id='search'
            className={cn(
          ...
          >
            <SearchFiltersItem itemType='collections' className='flex' onReset={(filterName?: string) => checkFiltering(filterName)} mobileBreakpoint={collectionsBreakpoint} itemTypeTitleOrName='title' />
            <div
              id='searchResults'
              className={cn(
            ...
            >
              <SearchResultsItem className='flex' apiTitleOrName='title' itemType='collections' showLabel={false} pageBreakpoint={collectionsBreakpoint} />
            </div>
          </div>
        ) : (
          <div
...
export const getServerSideProps: GetServerSideProps<ICollectionsPageProps> = async (context) => {
  return {
    props: {
