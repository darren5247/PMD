    ...
    <>
      {itemPage && itemPage.attributes.name ? (
        <Page
          ...
          description={
            itemPage.attributes.descriptionSEO
              ? itemPage.attributes.descriptionSEO
          ...
          image={
            itemPage.attributes.image.data?.attributes?.url
              ? itemPage.attributes.image.data.attributes.url
              : ""
          }
          className={cn(
          ...
          classNameMain={cn(
            ...
            itemPage.attributes.widthFull &&
              itemPage.attributes.showBackBar &&
              "!mt-[35px] lg:!mt-[35px] !max-w-full !mx-0 !px-0",
            itemPage.attributes.widthFull &&
              !itemPage.attributes.showBackBar &&
              "!mt-0 lg:!mt-0 !max-w-full !mx-0 !px-0",
          )}
        >
          <>
            {itemPage.attributes.name && !itemPage.attributes.hideName ? (
              <h1 className="mb-10 w-full">{itemPage.attributes.name}</h1>
            ) : (
              ""
            )}
                      ...
                      if (
                        ...
                        return (
                          <div
                            id={
                              domNode.attribs.id
                                ? "video-" + domNode.attribs.id
                                : "video-dQw4w9WgXcQ"
                            }
                            className="mx-auto mb-14 w-full md:max-w-[816px]"
                          >
                            <GetYouTubeVideo
                              videoId={
                                domNode.attribs.id
            ...
            {itemPage.attributes.showLastUpdated ? (
              <p
                className={cn(
        ...
        >
          <>
            ...
            <h2 className="mt-10">
              <em>This page was not able to be located.</em>
            </h2>
            <p className="mt-6 max-w-[810px]">
              The page you are looking for has moved, is no longer available,
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  try {
    ...
    const response = await pageApiGet.get((url as string) || "");
    return {
      props: {
