...
import { IconOpen, IconSearchRed, IconSearchWhite, IconFeedbackWhite, IconListenOnSpotify, IconListenOnAppleMusic } from '@src/common/assets/icons';
...
const CollectionDetailsPage: NextPage<ICollectionDetailsPageProps> = ({ musicCollection, prevUrl }) => {
  const router = useRouter();
  const { query } = router;
  const [works, setWorks] = useState<IStrapiPiece[]>([]);
  const [currentPagePieces, setCurrentPage] = useState(Number(query.page) && Number(query.page) > 0 ? Number(query.page) : 1);
  ...
  useEffect(
    () => {
      ...
      const getWorks = async () => {
        try {
          ...
          if (data?.meta?.pagination?.pageCount < currentPagePieces && data?.meta?.pagination?.pageCount > 0) {
            ...
            router.push({
              pathname: encodeURIComponent(handleTitleWithJustNumber(musicCollection.attributes.title)),
      ...
      getWorks();
    }, [dispatch, router, currentPagePieces, query, setWorks, setIsLoadingWorks, musicCollection?.id, musicCollection?.attributes?.title]
  );
    ...
    <>
      {musicCollection && musicCollection.attributes.title ? (
        <Page
          ...
          url={EUrlsPages.COLLECTION + '/' + encodeURIComponent(handleTitleWithJustNumber(musicCollection.attributes.title)) + '?id=' + musicCollection.id}
          prevUrl={prevUrl}
          title={musicCollection.attributes.title ? `${musicCollection.attributes.title} - Piano Music Database` : 'Collection - Piano Music Database'}
          description={musicCollection.attributes.description ? musicCollection.attributes.description : `Explore ${musicCollection.attributes.title} on Piano Music Database. Buy the sheet music, find the works that make up the collection, and discover other details.`}
          image={musicCollection.attributes.imageSEO?.data ? musicCollection.attributes.imageSEO.data?.attributes.url : ''}
        >
          <article className='flex flex-col justify-center items-center gap-y-3 text-center'>
            {!musicCollection.attributes.publishedAt && (
              <section id='review' className='flex justify-center'>
                <div className='bg-orange-500 shadow-workNav mx-4 mb-12 py-4 rounded-lg'>
                  ...
                  <p className='px-4 text-white text-center tracking-thigh'><strong>This collection is currently <em>under review</em> by staff.</strong></p>
                  <p className='mt-4 px-4 text-white text-center tracking-thigh'><strong>This page is publicly viewable while under review, but the collection will not appear in PMD search results.</strong></p>
                  ...
                  <p className='px-4 text-white text-center tracking-thigh'><strong>This includes related works, composers, and publishers which are under review.</strong></p>
                  <hr className='mt-4 mb-3' />
                  <div id='feedback' className='flex flex-row flex-wrap justify-center gap-x-5 gap-y-2 px-4 text-white text-center tracking-thigh'>
                    ...
                    <ModalFeedback
                      type={'DraftCollection - ' + EUrlsPages.COLLECTION + '/' + encodeURIComponent(handleTitleWithJustNumber(musicCollection.attributes.title)) + '?id=' + musicCollection.id}
                      url={EUrlsPages.COLLECTION + '/' + encodeURIComponent(handleTitleWithJustNumber(musicCollection.attributes.title)) + '?id=' + musicCollection.id}
                ...
                <div className='flex min-[935px]:flex-row flex-col justify-center items-top max-[935px]:items-center gap-x-6 gap-y-8'>
                  {musicCollection.attributes.image.data ? (
                    <div className='min-[935px]:mt-5 min-w-auto sm:min-w-[300px] max-w-[240px] sm:max-w-[300px]'>
                      <ImagePicture
                        alt={musicCollection.attributes.image.data?.attributes.alternativeText ? musicCollection.attributes.image.data?.attributes.alternativeText : musicCollection.attributes.title}
                        src={musicCollection.attributes.image.data?.attributes.url ? musicCollection.attributes.image.data?.attributes.url : ''}
                        height={musicCollection.attributes.image.data?.attributes.height ? musicCollection.attributes.image.data?.attributes.height : 0}
                        width={musicCollection.attributes.image.data?.attributes.width ? musicCollection.attributes.image.data?.attributes.width : 0}
                      />
                    </div>
                  ) : ''}
                  <div
                    className={cn(
                      ...
                      {
                        'pb-2 max-w-[700px] text-center justify-center items-center': !musicCollection.attributes.image?.data
                      }
                    )}
                  >
                    <h1
                    ...
                    {musicCollection.attributes.composers?.data?.length ? (
                      <div
                        className={cn(
                          ...
                          {
                            'text-center justify-center items-center': !musicCollection.attributes.image.data
                          }
                        )}>Composed {musicCollection.attributes.composed_date ? (`in ${musicCollection.attributes.composed_date}`) : ''} <span>by </span>
                        {musicCollection.attributes.composers.data.map((composer, composerIndex) => (
                          <span className='text-base' key={`${composer.attributes.name}-${composerIndex}`}>
                            ...
                            {musicCollection.attributes.composers.data && composerIndex < musicCollection.attributes.composers.data.length - 1 && ', '}
                          </span>
                        ))}</div>
                    ) : musicCollection.attributes.composed_date ? (<p
                      className={cn(
                        'flex flex-row flex-wrap gap-1 pr-1.5 mt-2 w-full mx-auto max-[935px]:justify-center [935px]:text-left',
                        {
                          'text-center justify-center items-center': !musicCollection.attributes.image.data
                        }
                      )}>Composed in {musicCollection.attributes.composed_date}</p>) : ''}
                    {musicCollection.attributes.publishers?.data?.length ? (
                      <div
                        ...
                        )}>Published {musicCollection.attributes.published_date ? (`in ${musicCollection.attributes.published_date}`) : ''} <span>by </span>
                        {musicCollection.attributes.publishers.data.map((publisher, publisherIndex) => (
                          <span className='text-base' key={`${publisher.attributes.name}-${publisherIndex}`}>
                            ...
                            {musicCollection.attributes.publishers.data && publisherIndex < musicCollection.attributes.publishers.data.length - 1 && ', '}
                          </span>
                        ))}</div>
                    ) : musicCollection.attributes.published_date ? (<p
                      className={cn(
                        'flex flex-row flex-wrap gap-1 pr-1.5 mt-2 w-full mx-auto max-[935px]:justify-center [935px]:text-left',
                        {
                          'text-center justify-center items-center': !musicCollection.attributes.image.data
                        }
                      )}>Published in {musicCollection.attributes.published_date}</p>) : ''}
                      ...
                      >
                        <div
                          ...
                          dangerouslySetInnerHTML={
                            {
                              __html: handleCleanContent(musicCollection.attributes.description)
                            }
                          }
                        />
                      </div>
                    ) : ''}
                    {musicCollection.attributes.catalogue_number ? (<p>Catalogue Number: {musicCollection.attributes.catalogue_number}</p>) : ''}
                    {(musicCollection.attributes.isbn_10 || musicCollection.attributes.isbn_13) ? (
                      <div className={cn(
                        ...
                        {musicCollection.attributes.isbn_10 ? (<p>ISBN-10: {musicCollection.attributes.isbn_10}</p>) : ''}
                        {musicCollection.attributes.isbn_13 ? (<p>ISBN-13: {musicCollection.attributes.isbn_13}</p>) : ''}
                      </div>
                    ) : ''}
                  </div>
                </div>
            ...
            {musicCollection.attributes.purchase_link?.length ? (
              <div id='get' className='flex flex-wrap justify-center items-center gap-4 mx-4 mt-8 mb-12'>
                {musicCollection.attributes.purchase_link.map((purchase_link) => (
                  <div key={purchase_link.url ? purchase_link.url : purchase_link.sellerName}>
                    <BuyLink sellerName={purchase_link.sellerName} itemName={`Sheet music for ${musicCollection.attributes.title}`} url={purchase_link.url} linkText={purchase_link.linkText} sellerImage={purchase_link.sellerImage.data?.attributes.url} sellerImageHeight={purchase_link.sellerImage.data?.attributes.height} sellerImageWidth={purchase_link.sellerImage.data?.attributes.width} />
                  </div>
                ))}
              </div>
            ) : ''}
            {(musicCollection.attributes.urlSpotify || musicCollection.attributes.urlAppleMusic) && (
                  ...
                  {(musicCollection.attributes.urlSpotify) && (
                    <div className='flex flex-row flex-wrap justify-center items-center gap-x-2 gap-y-1 shadow-musicCard p-1 rounded-lg text-center'>
                      <Link href={musicCollection.attributes.urlSpotify}>
                        <a className='group flex justify-center items-center text-center' title={`Listen to ${musicCollection.attributes.title} on Spotify`} target='_blank' rel='noopener noreferrer'>
                          <ImageNext
                  ...
                  {(musicCollection.attributes.urlAppleMusic) && (
                    <div className='flex flex-row flex-wrap justify-center items-center gap-x-2 gap-y-1 shadow-musicCard p-1 rounded-lg text-center'>
                      <Link href={musicCollection.attributes.urlAppleMusic}>
                        <a className='group flex justify-center items-center text-center' title={`Listen to ${musicCollection.attributes.title} on Apple Music`} target='_blank' rel='noopener noreferrer'>
                          <ImageNext
              ...
              <div id='website' className='mb-8'>
                <div className='flex flex-row flex-wrap justify-center items-center gap-x-2 gap-y-1 shadow-musicCard p-3 rounded-lg text-center'>
                  <Link href={musicCollection.attributes.urlWebsite}>
                    <a className='flex justify-center items-center text-center' title={musicCollection.attributes.urlWebsite} target='_blank' rel='noopener noreferrer'>
            ...
            {musicCollection.attributes.videoYouTube && (
              <>
                ...
                <div className='mx-auto mb-20 w-full md:max-w-[816px]'>
                  <VideoPlayer videoURL={musicCollection.attributes.videoYouTube} videoName={musicCollection.attributes.title} />
                </div>
              </>
            )}
            {isLoadingWorks ? (
              ...
              ((works.length > 0 && works.length) ? (
                <div className='flex flex-col justify-center items-center gap-1 mt-10 text-center align-middle'>
                  <ContentDivider title={works.length > 2 ? 'Pieces' : 'Piece'} className='mx-auto mb-1 md:max-w-[816px]' />
                  ...
                  <div className='flex flex-row flex-wrap justify-center items-center gap-3 mt-2 mb-4 max-w-[1200px] text-center align-middle'>
                    <Works works={works.slice(0, 10).map(piece => ({
                      id: piece.id,
                  ...
                  {works.length > 9 && (
                    <div className='flex justify-center items-center gap-3 min-[340px]:gap-5 mt-6 mb-3 text-sm'>
                      <Link
                        href={(
                          encodeURIComponent(handleTitleWithJustNumber(musicCollection.attributes.title)) +
                      ...
                      <Link
                        href={(
                          encodeURIComponent(handleTitleWithJustNumber(musicCollection.attributes.title)) +
                          ('?id=' + musicCollection.id + '&page=' + (currentPagePieces - 1))
                          + '#Pieces'
                        )}
                      >
                        <a
                      ...
                      <Link
                        href={(
                          encodeURIComponent(handleTitleWithJustNumber(musicCollection.attributes.title)) +
                          ('?id=' + musicCollection.id + '&page=' + totalPagesPieces)
                          + '#Pieces'
                        )}
                      >
                        <a
        ...
        (
          ...
          >
            <>
              ...
              <h2 className='mt-10'><em>This collection was not able to be located.</em></h2>
              <p className='mt-6 max-w-[864px]'>The collection you are looking for has moved, is no longer available, has been archived, or was not valid.</p>
              <p className='mt-2 max-w-[844px]'>If you were looking for a specific collection, make sure the URL is formatted like this: <br /><strong>/{EUrlsPages.COLLECTION}/TITLEHERE?id=IDHERE</strong> <br />The ID is a number we use to connect you to the correct collection. Some collections may have the same title, so we can not know which collection you are looking for without an ID in the URL.</p>
