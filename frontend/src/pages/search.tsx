...
import InstantSearchLoading from '@src/components/InstantSearchLoading';
...
const SearchPage: NextPage<ISearchPageProps> = ({ serverState, url, queries }) => {
  ...
  if (queriesDeEncoded) {
    pageTitle = 'Search for ' + capitalizeFirstLetter(queriesDeEncoded) + ' on Piano Music Database';
    pageDesc = 'Search for ' + capitalizeFirstLetter(queriesDeEncoded) + ' on Piano Music Database. Piano Music Database is a search engine and database of pedagogical repertoire (level, element, mood, style, and more) built for piano teachers. Find the perfect piece on PianoMusicDatabase.com.';
  } else {
    pageTitle = 'Search Piano Music Database - Find the Perfect Piece'
    pageDesc = 'Piano Music Database is a search engine and database of pedagogical repertoire (level, element, mood, style, and more) built for piano teachers. Find the perfect piece on PianoMusicDatabase.com.'
  };
  ...
  return (
    <InstantSearchSSRProvider {...serverState}>
      ...
      <Layout className='flex scrollbar'>
        ...
        >
          {/* {typeof isBreakpoint === 'boolean' && <InstantSearchLoading />} */}
          <Header showBackBar={true} showBackBarFeedback={true} classNameSearchForm='pt-2' />
          <Content>
...
export const getServerSideProps: GetServerSideProps<ISearchPageProps> = async ({ req }) => {
