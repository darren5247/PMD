  ...
  return (
    ...
    >
      <div className='flex flex-col justify-center items-center mt-6 text-center'>
        ...
        <p className='mt-3 mb-3 max-w-[870px] text-justify'>Piano Music Database uses <strong>7 levels</strong> that increase in difficulty starting with the simplest pre-reading piano music and ending with the most virtuosic repertoire for professionals. <br />PMD levels are broad compared to other leveling systems. <br />The goal with our leveling system is to organize a piece into a difficulty range and to use its <Link href={`/${EUrlsPages.ELEMENTS}`}><a title='Learn more about our elements'>elements</a></Link> to give the full picture of its difficulty. This page goes over our leveling system and each level in the system in great detail. It explains how we determine the difference between the levels, the common <Link href={`/${EUrlsPages.ELEMENTS}`}><a title='Learn more about our elements'>elements</a></Link> found in each level, and lists a few representative pieces for each level.</p>
        <p className='mt-6 mb-9 text-sm text-center italic'>Use the level selector at the top of this page to jump to a specific level quickly and learn more about it.</p>
        {isLoading ? (
          <p>Loading levels...</p>
        ) : (
          (levels ? (
            <div className='flex flex-row flex-wrap justify-center items-top gap-3 max-w-[1200px] text-center align-middle'>
              {levels.map((level) => (
                <CardLevelDesc
                  ...
                  id={level.id}
                  isSearchable={level.attributes.isSearchable}
                />
              ))}
            </div>
          ) : '')
